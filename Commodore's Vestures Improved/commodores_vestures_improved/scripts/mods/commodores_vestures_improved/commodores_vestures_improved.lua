...
local Definitions = mod:io_dofile("commodores_vestures_improved/scripts/mods/commodores_vestures_improved/commodores_vestures_improved_definitions")
...
local StoreItemDetailViewDefinitions = require("scripts/ui/views/store_item_detail_view/store_item_detail_view_definitions")
...
mod:hook_safe(
    CLASS.StoreItemDetailView, "cb_on_preview_with_gear_toggled", function(self)
...
mod:hook_safe(
    CLASS.CosmeticsInspectView, "on_enter", function(self)
        mod._refresh_profiles(self)
...
mod:hook_require(
    "scripts/ui/views/cosmetics_inspect_view/cosmetics_inspect_view_definitions", function(instance)
...
CosmeticsInspectView._setup_item_description = function(self, description_text, restriction_text, property_text)
    ...
    if restriction_text then
        ...
        if selected_profile then
            _add_text_widget(CosmeticsInspectViewDefinitions.item_sub_title_pass, Utf8.upper(Localize("loc_CVI_currently_showing")))
            _add_spacing(10)
            local text
...
-- For item bundles
mod:hook_safe(
    CLASS.StoreItemDetailView, "_present_bundle", function(self)
...
mod._refresh_profiles = function(self)
    ...
    Managers.data_service.profiles:fetch_all_profiles():next(
    ...
    if not self._weapon_preview then
        ...
        if is_bundle and #self._items > 1 then
            ...
            if not hide_restrictions then
                if len_bundle_restrictions > 1 then
                    ...
                    _add_text_widget(StoreItemDetailViewDefinitions.item_sub_title_pass, Utf8.upper(Localize("loc_item_equippable_on_header")))
                    _add_spacing(8)
                ...
                if self._previewed_with_gear then
                    ...
                    _add_text_widget(StoreItemDetailViewDefinitions.item_sub_title_pass, Utf8.upper(Localize("loc_CVI_currently_showing")))
                    _add_spacing(8)
                    local text
        ...
        if item and not is_bundle then
            ...
            if properties_text then
                ...
                _add_text_widget(StoreItemDetailViewDefinitions.premium_sub_title_pass, Utf8.upper(Localize("loc_item_property_header")))
                _add_spacing(8)
        ...
        if item then
            ...
            if allowed_archetypes then
                ...
                for i, allowed_archetype in pairs(allowed_archetypes) do
                    if self._presentation_profile and self._presentation_profile.archetype.name == allowed_archetype then
