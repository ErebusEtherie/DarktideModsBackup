# Native DIY interface catalog

Fixed to game Lua 1.12.5, commit `0f0cb45991e9305ef4a7b925370792d7d6035f95`. Identifiers and types are audited from the source. Presence does not guarantee every consumer, weapon, class or special state supports a given combination.

Read GUIDE.en.md before editing. Copy exact identifiers; the loader rejects unknown names. The machine-readable files are native-catalog.json and event-contracts.json. The latter records each native event separately; only the scalar whitelist can be used as event.FIELD conditions.
...
## Condition-readable event scalars (95)
...
## Audited native status names (15)
...
## Ability name registry (Blitz filter uses equipped name) (61)
