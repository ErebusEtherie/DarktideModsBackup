...
return function(mod)
	...
	---@field permanent_damage_taken fun(self:DL_HealthExtension):number
	---@field permanent_damage_taken_percent fun(self:DL_HealthExtension):number
	---@field num_wounds fun(self:DL_HealthExtension):integer
	---@field max_wounds fun(self:DL_HealthExtension):integer

	---@class DL_ToughnessExtension
	---@field remaining_toughness fun(self:DL_ToughnessExtension):number
	---@field max_toughness_visual fun(self:DL_ToughnessExtension):number
	...
	---@alias DL_NamedExtension "health_system" | "toughness_system" | "buff_system" | "unit_data_system" | "visual_loadout_system"
	...
	function Player.extensions(unit, ...)
		...
		local has_extension = FS_ScriptUnit.has_extension
		local count = select("#", ...)

		if count == 1 then
			return has_extension(unit, ...)
		elseif count == 2 then
			local first, second = ...
			return has_extension(unit, first), has_extension(unit, second)
		elseif count == 3 then
			local first, second, third = ...
			return has_extension(unit, first), has_extension(unit, second), has_extension(unit, third)
		end
		...
		for i = 1, count do
			local name = select(i, ...)
			results[i] = has_extension(unit, name)
		end
	...
	---@param player_or_unit DL_PlayerObject | Unit | nil
	---@return number | nil
	function Player.corruption(player_or_unit)
		local ext = health_extension(player_or_unit)
		return ext and ext:permanent_damage_taken() or nil
	end

	---@param player_or_unit DL_PlayerObject | Unit | nil
	---@return number | nil
	function Player.corruption_percent(player_or_unit)
		local ext = health_extension(player_or_unit)
		return ext and ext:permanent_damage_taken_percent() or nil
	end

	---@param player_or_unit DL_PlayerObject | Unit | nil
	---@return integer | nil
	function Player.wounds(player_or_unit)
		local ext = health_extension(player_or_unit)
		return ext and ext:num_wounds() or nil
	end

	---@param player_or_unit DL_PlayerObject | Unit | nil
	---@return integer | nil
	function Player.max_wounds(player_or_unit)
		local ext = health_extension(player_or_unit)
		return ext and ext:max_wounds() or nil
	end
	...
	---@param player_or_unit DL_PlayerObject | Unit | nil
	---@return number | nil
	function Player.max_toughness_visual(player_or_unit)
		local ext = toughness_extension(player_or_unit)
		return ext and ext.max_toughness_visual and ext:max_toughness_visual() or 0
	end

	---@param player_or_unit DL_PlayerObject | Unit | nil
	---@return boolean | nil
	function Player.has_golden_toughness(player_or_unit)
		local ext = toughness_extension(player_or_unit)
		...
		local toughness_percentage = ext and ext:current_toughness_percent() or 0
		local max_toughness = ext and ext.max_toughness and ext:max_toughness() or 0
		local max_toughness_visual = ext.max_toughness_visual and ext:max_toughness_visual() or 0
		...
		local overshield_amount = current_toughness_visual < max_toughness
				and math.max(current_toughness - max_toughness_visual, 0)
			or 0
		local has_overshield = math.floor(overshield_amount) > 0

		return has_overshield
	end
