...
if ui_owner._ui_class_RealmsInventoryView then
    return ui_owner._ui_class_RealmsInventoryView
end
...
local VIEW_BY_SLOT = {
	...
	slot_attachment_1 = "realms_inventory_weapons_view",
	slot_attachment_2 = "realms_inventory_weapons_view",
	slot_attachment_3 = "realms_inventory_weapons_view",
	...
	slot_primary = "realms_inventory_weapons_view",
	slot_secondary = "realms_inventory_weapons_view"
}
local DIRECTION = {
	DOWN = "down",
	LEFT = "left",
...
local RealmsInventoryView = class("RealmsInventoryView", "BaseView")

function RealmsInventoryView:init(settings, context)
	...
	RealmsInventoryView.super.init(self, Definitions, settings, context)
...
function RealmsInventoryView:on_enter()
	RealmsInventoryView.super.on_enter(self)
...
function RealmsInventoryView:_get_inventory_item_by_id(gear_id)
	if not gear_id then
		return
	end
...
function RealmsInventoryView:_setup_item_stats(reference_name, scenegraph_id)
	local layer = 10
...
function RealmsInventoryView:_update_item_stats_position(scenegraph_id, item_stats)
	local position = self:_scenegraph_world_position(scenegraph_id)

	item_stats:set_pivot_offset(position[1], position[2])
end

function RealmsInventoryView:_on_item_hover_start(item)
...
function RealmsInventoryView:_on_item_hover_stop()
	self._currently_hovered_item = nil
...
function RealmsInventoryView:_display_title_text(text)
...
function RealmsInventoryView:_set_active_layout_by_index(index)
...
function RealmsInventoryView:_switch_active_layout(tab_context)
...
function RealmsInventoryView:supports_changeable_context()
	return true
end

function RealmsInventoryView:on_back_pressed()
...
function RealmsInventoryView:on_exit()
	if self._item_stats then
		self._item_stats:stop_presenting()
	...
	if Managers.ui:view_active("realms_inventory_weapons_view") then
		Managers.ui:close_view("realms_inventory_weapons_view")
	end

	self:_destroy_offscreen_gui()
	RealmsInventoryView.super.on_exit(self)
end

function RealmsInventoryView:_setup_offscreen_gui()
...
function RealmsInventoryView:_destroy_offscreen_gui()
...
function RealmsInventoryView:_fetch_inventory_items()
...
function RealmsInventoryView:_item_valid_by_current_profile(item)
...
function RealmsInventoryView:_get_items_layout_by_slot(slot)
...
function RealmsInventoryView:event_inventory_view_set_camera_focus(slot_name)
	self:_set_camera_focus_by_slot_name(slot_name)
end

function RealmsInventoryView:_set_camera_focus_by_slot_name(slot_name, optional_camera_settings, force_instant_camera)
	if slot_name then
...
function RealmsInventoryView:event_force_wallet_update()
	self:_request_wallets_update()
end

function RealmsInventoryView:cb_on_grid_entry_right_pressed(widget, element)
...
function RealmsInventoryView:player()
	return self._preview_player
end

function RealmsInventoryView:equipped_item_in_slot(slot_name)
	return self._preview_profile_equipped_items[slot_name]
end

function RealmsInventoryView:cb_on_grid_entry_pressed(widget, element)
	if not self._is_own_player or self._is_readonly then
		return
	end
...
function RealmsInventoryView:_play_voice_preview(slot_name, item)
...
function RealmsInventoryView:_clear_widgets(widgets)
	if widgets then
		for i = 1, #widgets do
...
function RealmsInventoryView:_set_group_header_highlight(group_header, text)
	self._highlighted_group_header = group_header
	self._highlighted_group_header_text = text
end

function RealmsInventoryView:get_group_header_highlight_text(group_header)
...
function RealmsInventoryView:_reset_grid_layout(ui_renderer)
...
function RealmsInventoryView:_setup_individual_layout(layout)
...
function RealmsInventoryView:_setup_grid_layout(layout)
...
function RealmsInventoryView:cb_switch_tab(index)
	if index ~= self._selected_tab_index then
		self:_set_active_layout_by_index(index)
	end
end

function RealmsInventoryView:_destroy_previous_tab()
	if self._wallet_animation_id then
		self:_stop_animation(self._wallet_animation_id)
...
function RealmsInventoryView:_setup_menu_tabs(content)
	local id = "tab_menu"
	local layer = 10
...
function RealmsInventoryView:_update_tab_bar_position()
...
function RealmsInventoryView:_setup_grid(widgets, alignment_list, grid_scenegraph_id, spacing)
	local direction = "down"

	return UIWidgetGrid:new(widgets, alignment_list, self._ui_scenegraph, grid_scenegraph_id, direction, spacing)
end

function RealmsInventoryView:_create_entry_widget_from_config(config, suffix, callback_name, secondary_callback_name, optional_scenegraph_id)
...
function RealmsInventoryView:_create_exclamation_widget_from_config(config, suffix, scenegraph_id)
	local widget_type = config.widget_type
	local template = ContentBlueprints[widget_type]
	local size = template.size_function and template.size_function(self, config) or template.size
...
function RealmsInventoryView:_draw_loadout_widgets(dt, t, input_service, ui_renderer)
...
function RealmsInventoryView:_draw_wallet_widgets(dt, t, input_service, ui_renderer)
...
function RealmsInventoryView:_draw_grid(dt, t, input_service, ui_renderer)
...
function RealmsInventoryView:_draw_elements(dt, t, ui_renderer, render_settings, input_service)
	...
	RealmsInventoryView.super._draw_elements(self, dt, t, ui_renderer, render_settings, input_service)

	render_settings.alpha_multiplier = previous_alpha_multiplier
end

function RealmsInventoryView:_destroy_loadout_widgets(ui_renderer)
	local widgets = self._loadout_widgets
...
function RealmsInventoryView:_destroy_grid_widgets(ui_renderer)
	local widgets = self._grid_widgets
...
function RealmsInventoryView:_update_blueprint_widgets(widgets, dt, t, input_service, ui_renderer)
	if widgets then
...
function RealmsInventoryView:_handle_input(input_service)
...
function RealmsInventoryView:_get_next_array_index(direction, start_index, array)
...
function RealmsInventoryView:_first_interactable_loadout_widget_index()
...
function RealmsInventoryView:_find_closest_widget_node_neighbour(direction)
...
function RealmsInventoryView:_get_coordinates_from_widget(widget)
...
function RealmsInventoryView:_find_closest_value(list, start_coordinates, direction, threshold, current_index, result_index)
...
function RealmsInventoryView:_handle_individual_widget_selection(input_service)
	local widget_index
...
function RealmsInventoryView:_on_navigation_input_changed()
	RealmsInventoryView.super._on_navigation_input_changed(self)
...
function RealmsInventoryView:_select_individual_widget_index(index)
...
function RealmsInventoryView:block_input(is_blocking)
	self._parent_block_input = is_blocking
end

function RealmsInventoryView:on_resolution_modified(scale)
	RealmsInventoryView.super.on_resolution_modified(self, scale)
...
function RealmsInventoryView:update(dt, t, input_service)
	...
	self._weapon_view_open = Managers.ui:view_active("realms_inventory_weapons_view")
	...
	return RealmsInventoryView.super.update(self, dt, t, input_service)
end

function RealmsInventoryView:draw(dt, t, input_service, layer)
	if self._parent_block_input or self._parent and self._parent.is_inventory_synced and not self._parent:is_inventory_synced() then
		input_service = input_service:null_service()
	end

	RealmsInventoryView.super.draw(self, dt, t, input_service, layer)
...
function RealmsInventoryView:_request_wallets_update()
	...
	if preview_player and preview_player == local_player then
		...
		promise:next(function(wallets_data)
			...
			Log.error("RealmsInventoryView", "Failed to fetch player wallets")
...
function RealmsInventoryView:_update_wallets_presentation(wallets_data, hide_presentation)
...
function RealmsInventoryView:profile_level()
	local preview_player = self._preview_player
...
function RealmsInventoryView:on_item_stats_toggled(force_value, do_save)
	...
	-- Item stats toggle is intentionally not persisted to official save data.
end

function RealmsInventoryView:is_item_stats_toggled()
	return self._item_stats_presentation_enabled
end

ui_owner._ui_class_RealmsInventoryView = RealmsInventoryView
return RealmsInventoryView