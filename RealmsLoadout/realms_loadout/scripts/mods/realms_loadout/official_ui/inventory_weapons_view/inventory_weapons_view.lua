...
if ui_owner._ui_class_RealmsInventoryWeaponsView then
    return ui_owner._ui_class_RealmsInventoryWeaponsView
end
...
local InventoryWeaponStatsViewElement = require("realms_loadout/scripts/mods/realms_loadout/official_ui/inventory_weapons_view/inventory_weapon_stats_view_element")
local RealmsInventoryWeaponsView = class("RealmsInventoryWeaponsView", "ItemGridViewBase")
...
local function is_local_weapon_slot(slot_name)
	return slot_name == "slot_primary" or slot_name == "slot_secondary"
end

local function is_attachment_slot(slot_name)
	return slot_name and ATTACHMENT_SLOTS[slot_name] == true
end

local function item_source_gear_id(item)
	if type(item) ~= "table" then
		return item
	end

	local catalog_entry = item.__weapon_catalog_entry or item.__attachment_catalog_entry

	return catalog_entry and catalog_entry.source_gear_id
		or item.source_gear_id
		or item.gear_id
		or item.__gear_id
end

function RealmsInventoryWeaponsView:_uses_local_weapon_catalog()
	local selected_slot = self._selected_slot
	local slot_name = selected_slot and selected_slot.name

	if is_local_weapon_slot(slot_name) then
		return WeaponCatalog.local_mode_enabled()
	end

	if is_attachment_slot(slot_name) then
		return AttachmentCatalog.local_mode_enabled()
	end

	return false
end

function RealmsInventoryWeaponsView:init(settings, context)
	...
	RealmsInventoryWeaponsView.super.init(self, Definitions, settings, context)
...
function RealmsInventoryWeaponsView:on_enter()
	RealmsInventoryWeaponsView.super.on_enter(self)
...
function RealmsInventoryWeaponsView:event_switch_mark_complete(item)
	local selected_slot = self._selected_slot
...
function RealmsInventoryWeaponsView:_setup_background_frames_by_archetype(profile)
...
function RealmsInventoryWeaponsView:_setup_item_grid_materials()
...
function RealmsInventoryWeaponsView:_setup_weapon_actions()
...
function RealmsInventoryWeaponsView:on_resolution_modified(scale)
	RealmsInventoryWeaponsView.super.on_resolution_modified(self, scale)
	self:_update_weapon_actions_position()
...
function RealmsInventoryWeaponsView:_update_weapon_actions_position()
	if not self._weapon_actions then
		return
	end
...
function RealmsInventoryWeaponsView:_setup_input_legend()
...
function RealmsInventoryWeaponsView:_register_button_callbacks()
	local widgets_by_name = self._widgets_by_name
...
function RealmsInventoryWeaponsView:cb_switch_tab(index)
	RealmsInventoryWeaponsView.super.cb_switch_tab(self, index)
end

function RealmsInventoryWeaponsView:cb_on_discard_pressed()
...
function RealmsInventoryWeaponsView:cb_on_discard_button_pressed()
	...
	if self._discard_items_element:visible() then
		...
		if self:_uses_local_weapon_catalog() then
			local profile = self._preview_player and self._preview_player:profile()
			local source_gear_ids = {}

			for i = 1, #gear_ids do
				local item = self._inventory_items and self._inventory_items[gear_ids[i]]
				local catalog_entry = item and item.__weapon_catalog_entry
				local source_gear_id = catalog_entry and catalog_entry.source_gear_id or item and item.source_gear_id

				if source_gear_id then
					source_gear_ids[#source_gear_ids + 1] = source_gear_id
				end
			end

			if profile then
				WeaponCatalog.mark_local_weapons_removed(profile, source_gear_ids)
			end

			self._selected_items = {}
			self:cb_on_discard_pressed()
			self:_fetch_local_weapon_items(self._selected_slot)
		else
...
function RealmsInventoryWeaponsView:_mark_items_to_sell(items)
...
function RealmsInventoryWeaponsView:_unmark_items_to_sell(items)
	if items and #items > 0 then
		local widgets = self._item_grid:widgets()
...
function RealmsInventoryWeaponsView:_item_hover_update()
...
function RealmsInventoryWeaponsView:cb_on_favorite_pressed()
	local widget
...
function RealmsInventoryWeaponsView:cb_on_grid_entry_left_pressed(widget, element)
	if self._discard_items_element then
...
function RealmsInventoryWeaponsView:is_selected_item_equipped()
...
function RealmsInventoryWeaponsView:is_selected_item_favorited()
	local selected_grid_widget = self:selected_grid_widget()
...
function RealmsInventoryWeaponsView:cb_on_close_pressed()
	self:_handle_back_pressed()
end

function RealmsInventoryWeaponsView:_preview_element(element)
	RealmsInventoryWeaponsView.super._preview_element(self, element)
end

function RealmsInventoryWeaponsView:sort_items()
	local sort_options = self._sort_options
...
function RealmsInventoryWeaponsView:update(dt, t, input_service)
	...
	return RealmsInventoryWeaponsView.super.update(self, dt, t, input_service)
end

function RealmsInventoryWeaponsView:_handle_input(input_service)
	if not self._using_cursor_navigation then
...
function RealmsInventoryWeaponsView:_handle_back_pressed()
	if not self._discard_items_element then
		local view_name = "realms_inventory_weapons_view"
...
function RealmsInventoryWeaponsView:cb_on_customize_pressed()
...
function RealmsInventoryWeaponsView:cb_on_options_pressed()
...
function RealmsInventoryWeaponsView:cb_on_sync_official_weapon_pressed()
	if not self._is_own_player then
		return
	end

	if not self:_uses_local_weapon_catalog() then
		return
	end

	if self._sync_official_weapon_pending then
		return
	end
	...
	local catalog_entry = previewed_item.__weapon_catalog_entry

	if not catalog_entry or catalog_entry.origin ~= "official" then
		mod:notify(mod:localize("sync_official_weapon_unsupported"))

		return
	end

	local source_gear_id = catalog_entry.source_gear_id
	local selected_slot = self._selected_slot

	if not source_gear_id or not selected_slot then
		mod:notify(mod:localize("sync_official_weapon_error"))
	...
	if not profile or not gear_service or type(gear_service.fetch_inventory) ~= "function" then
		mod:notify(mod:localize("sync_official_weapon_error"))

		return
	end

	self._sync_official_weapon_pending = true

	local ok, promise = pcall(function ()
		return gear_service:fetch_inventory(player:character_id(), { slot_name })
	end)

	if not ok or not promise or type(promise.next) ~= "function" then
		self._sync_official_weapon_pending = false
		mod:notify(mod:localize("sync_official_weapon_error"))
	...
	promise:next(function (items)
		...
		for _, item in pairs(items or {}) do
			if type(item) == "table" and item.gear_id == source_gear_id then
				official_item = item
		...
		if not official_item then
			self._sync_official_weapon_pending = false
			mod:notify(mod:localize("sync_official_weapon_missing"))

			return
		end

		local new_entry = WeaponCatalog.sync_weapon_from_official(profile, source_gear_id, official_item)
		...
		local new_item = WeaponCatalog.make_local_weapon_instance(new_entry, slot_name)
		...
		-- Refresh the currently previewed item immediately. Replacing only the
		-- grid entry left the old preview/stats on screen until the view was
		-- reopened and the list was rebuilt from disk.
		self._previewed_item = new_item
		self:event_replace_list_item(new_item)

		local equipped_item = self:equipped_item_in_slot(slot_name)

		if equipped_item and equipped_item.gear_id == new_item.gear_id then
			Managers.event:trigger("event_inventory_view_equip_item", slot_name, new_item, true)
		end

		mod:notify(mod:localize("sync_official_weapon_success"))
	end):catch(function ()
		self._sync_official_weapon_pending = false
	end)
end

function RealmsInventoryWeaponsView:_on_navigation_input_changed()
...
function RealmsInventoryWeaponsView:cb_on_inspect_pressed()
	if not Managers.ui:view_active("realms_inventory_weapon_details_view") then
		self._inpect_view_opened = true

		Managers.ui:open_view("realms_inventory_weapon_details_view", nil, nil, nil, nil, {
			player = self._preview_player,
			preview_item = self._previewed_item
		})
	end
...
function RealmsInventoryWeaponsView:cb_on_marks_pressed()
	if not Managers.ui:view_active("realms_inventory_weapon_marks_view") then
		self._marks_view_opened = true

		Managers.ui:open_view("realms_inventory_weapon_marks_view", nil, nil, nil, nil, {
			player = self._preview_player,
			preview_item = self._previewed_item
		})
	end
...
function RealmsInventoryWeaponsView:cb_on_equip_pressed()
	local selected_slot = self._selected_slot
...
function RealmsInventoryWeaponsView:_setup_background_world()
...
function RealmsInventoryWeaponsView:world_spawner()
	return self._world_spawner
end

function RealmsInventoryWeaponsView:on_exit()
	...
	if self._inpect_view_opened then
		...
		if Managers.ui:view_active("realms_inventory_weapon_details_view") then
			Managers.ui:close_view("realms_inventory_weapon_details_view")
		end
	end
	...
	RealmsInventoryWeaponsView.super.on_exit(self)
end

function RealmsInventoryWeaponsView:_fetch_inventory_items(selected_slot)
	if self:_uses_local_weapon_catalog() then
		return Promise.resolved():next(function ()
			self:_fetch_local_weapon_items(selected_slot)
		end)
	end
...
function RealmsInventoryWeaponsView:_fetch_local_weapon_items(selected_slot)
	local player = self._preview_player or Managers.player:local_player(1)
	local profile = player and player:profile()
	local slot_name = selected_slot.name
	local local_items = {}

	if profile and is_local_weapon_slot(slot_name) then
		local_items = WeaponCatalog.local_instances(profile, slot_name)
	elseif profile and is_attachment_slot(slot_name) then
		local_items = AttachmentCatalog.local_instances(profile, slot_name)
	end

	if (is_attachment_slot(slot_name) or #local_items == 0 and is_local_weapon_slot(slot_name)) and not self._local_weapon_catalog_refresh_pending and player then
		local gear_service = Managers.data_service and Managers.data_service.gear

		if gear_service and type(gear_service.fetch_inventory) == "function" then
			self._local_weapon_catalog_refresh_pending = true

			local ok, promise = pcall(function ()
				return gear_service:fetch_inventory(player:character_id())
			end)

			if ok and promise and type(promise.next) == "function" then
				promise:next(function (official_items)
					if self._destroyed then
						return
					end

					if is_attachment_slot(slot_name) then
						AttachmentCatalog.sync_from_official(profile, official_items)
					else
						WeaponCatalog.sync_from_official(profile, official_items)
					end

					self:_fetch_local_weapon_items(selected_slot)
				end):catch(function ()
					if not self._destroyed then self:_fetch_local_weapon_items(selected_slot) end
				end)

				return
			end

			self._local_weapon_catalog_refresh_pending = false
		end
	end
	local items = {}

	for i = 1, #local_items do
		local item = local_items[i]

		items[item.gear_id] = item
	end
	...
	local slot_display_name = selected_slot and selected_slot.display_name
	local equipped_item = #layout > 0 and self:equipped_item_in_slot(slot_name)
	local selected_layout_item

	if equipped_item then
		local equipped_source_gear_id = item_source_gear_id(equipped_item)

		for i = 1, #layout do
			local layout_item = layout[i].item

			if item_source_gear_id(layout_item) == equipped_source_gear_id then
				selected_layout_item = layout_item
	...
	if selected_layout_item then
		self._selected_gear_id = selected_layout_item.gear_id
...
function RealmsInventoryWeaponsView:_setup_weapon_options()
	if Items.is_weapon(self.item_type) then
		...
		if self:_uses_local_weapon_catalog() then
			local sync_button = {
				widget_type = "button",
				display_name = mod:localize("sync_official_weapon"),
				callback = callback(self, "cb_on_sync_official_weapon_pressed"),
				mod_source = "realms_loadout_sync_official_weapon"
			}
			local exists = false

			for i = 1, #layout do
				if layout[i].mod_source == sync_button.mod_source then
					exists = true
			...
			if not exists then
				table.insert(layout, 1, sync_button)
			end
		end

		local button_count = #layout
		local grid_size = {
			button_size[1],
			(button_size[2] + 20) * button_count + top_padding
		}
		local grid_options = {
			edge_padding = 40,
			scrollbar_width = 7,
...
function RealmsInventoryWeaponsView:_calc_text_size(widget, text_and_style_id)
...
function RealmsInventoryWeaponsView:event_replace_list_item(item)
...
function RealmsInventoryWeaponsView:_get_offer_item_layout_by_gear_id(gear_id)
...
function RealmsInventoryWeaponsView:replace_item_instance(item)
	local gear_id = item.gear_id
...
function RealmsInventoryWeaponsView:_get_item_from_inventory(wanted_item)
	local inventory_items = self._inventory_items
	...
	if self:_uses_local_weapon_catalog() then
		local wanted_source_gear_id = item_source_gear_id(wanted_item)

		if wanted_source_gear_id then
			for _, item in pairs(inventory_items) do
				if item_source_gear_id(item) == wanted_source_gear_id then
					return item
				end
			end
		end

		local wanted_gear_id = type(wanted_item) == "table" and wanted_item.gear_id or wanted_item

		if wanted_gear_id and inventory_items[wanted_gear_id] then
			return inventory_items[wanted_gear_id]
		end
...
function RealmsInventoryWeaponsView:_item_valid_by_current_profile(item)
	local player = self._preview_player
	...
	local breed_valid
	local archetype_valid

	if allow_all_archetype_weapons and WeaponCatalog.is_human_profile(profile) then
		archetype_valid = true
		breed_valid = WeaponCatalog.is_human_like_weapon(item)
	else
		archetype_valid = not item.archetypes or table.contains(item.archetypes, archetype_name)
		breed_valid = not item.breeds or table.contains(item.breeds, breed_name)
	end
...
function RealmsInventoryWeaponsView:_on_double_click(widget, element)
	local selected_slot = self._selected_slot
...
function RealmsInventoryWeaponsView:_fetch_item_compare_slot_name(item)
	local selected_slot = self._selected_slot
...
function RealmsInventoryWeaponsView:_equip_item(slot_name, item)
...
function RealmsInventoryWeaponsView:has_equipped_item_in_any_slot(slots)
	for i = 1, #slots do
...
function RealmsInventoryWeaponsView:is_item_equipped_in_any_slot(item, slots)
	for i = 1, #slots do
		local slot_name = slots[i]
		local equipped_item = self:equipped_item_in_slot(slot_name)
...
function RealmsInventoryWeaponsView:equipped_item_in_slot(slot_name)
...
function RealmsInventoryWeaponsView:event_discard_items(gear_ids)
	local inventory_items = self._inventory_items
...
function RealmsInventoryWeaponsView:_stop_previewing()
	RealmsInventoryWeaponsView.super._stop_previewing(self)
...
function RealmsInventoryWeaponsView:_preview_item(item)
	RealmsInventoryWeaponsView.super._preview_item(self, item)
...
function RealmsInventoryWeaponsView:_update_equip_button_status()
	local previewed_item = self._previewed_item
...
function RealmsInventoryWeaponsView:_update_discard_button_status(dt)
...
function RealmsInventoryWeaponsView:_cb_on_present()
	RealmsInventoryWeaponsView.super._cb_on_present(self)
...
function RealmsInventoryWeaponsView:cb_on_toggle_item_compare()
	RealmsInventoryWeaponsView.super.cb_on_toggle_item_compare(self)
...
function RealmsInventoryWeaponsView:_setup_weapon_stats(reference_name, scenegraph_id)
	local layer = 1
	local context = self._definitions.weapon_stats_grid_settings
	local weapon_stats = self:_add_element(InventoryWeaponStatsViewElement, reference_name, layer, context)
...
function RealmsInventoryWeaponsView:_update_weapon_stats_position(scenegraph_id, weapon_stats)
	RealmsInventoryWeaponsView.super._update_weapon_stats_position(self, scenegraph_id, weapon_stats)
...
ui_owner._ui_class_RealmsInventoryWeaponsView = RealmsInventoryWeaponsView
return RealmsInventoryWeaponsView
