...
if ui_owner._ui_class_RealmsForgeViewElementWeaponStats then
    return ui_owner._ui_class_RealmsForgeViewElementWeaponStats
end
...
local _create_definitions_function = require("realms_loadout/scripts/mods/realms_loadout/official_ui/weapon_forge_view/forge_view_element_weapon_stats_definitions")
local ForgeViewElementWeaponStats = class("RealmsForgeViewElementWeaponStats", "ViewElementGrid")

function ForgeViewElementWeaponStats:init(parent, draw_layer, start_scale, optional_menu_settings)
	...
	ForgeViewElementWeaponStats.super.init(self, parent, draw_layer, start_scale, optional_menu_settings, definitions)
...
function ForgeViewElementWeaponStats:destroy(ui_renderer)
	ForgeViewElementWeaponStats.super.destroy(self, ui_renderer)
end
...
function ForgeViewElementWeaponStats:_verify_weapon(item)
	if not item then
		return false
	end
...
function ForgeViewElementWeaponStats:present_item(item, context, on_present_callback)
...
function ForgeViewElementWeaponStats:stop_presenting()
...
function ForgeViewElementWeaponStats:is_presenting()
	return not not self._item
end

function ForgeViewElementWeaponStats:present_grid_layout(layout, on_present_callback, item)
	...
	local generate_blueprints_function = require("realms_loadout/scripts/mods/realms_loadout/official_ui/weapon_forge_view/forge_item_stats_blueprints")
	local menu_settings = self._menu_settings
	local grid_size = menu_settings.grid_size
	local mask_size = menu_settings.mask_size
	...
	ForgeViewElementWeaponStats.super.present_grid_layout(self, layout, ContentBlueprints, left_click_callback, right_click_callback, grid_display_name, grow_direction, on_present_callback)
end

function ForgeViewElementWeaponStats:play_expertise_upgrade_animation(callback)
	return self:_start_animation("on_expertise_upgrade", self._widgets_by_name, nil, callback)
end

function ForgeViewElementWeaponStats:_on_present_grid_layout_changed(layout, content_blueprints, left_click_callback, right_click_callback, display_name, optional_grow_direction)
	ForgeViewElementWeaponStats.super._on_present_grid_layout_changed(self, layout, content_blueprints, left_click_callback, right_click_callback, display_name, optional_grow_direction)
...
function ForgeViewElementWeaponStats:draw(dt, t, ui_renderer, render_settings, input_service)
	self._stored_ui_renderer = ui_renderer

	ForgeViewElementWeaponStats.super.draw(self, dt, t, ui_renderer, render_settings, input_service)
end

function ForgeViewElementWeaponStats:update(dt, t, input_service)
	return ForgeViewElementWeaponStats.super.update(self, dt, t, input_service)
end

function ForgeViewElementWeaponStats:select_perk(index)
...
function ForgeViewElementWeaponStats:select_trait(index)
	local widgets = self:widgets()
...
function ForgeViewElementWeaponStats:preview_perk(index, new_perk)
	local widgets = self:widgets()
...
function ForgeViewElementWeaponStats:preview_trait(index, new_trait)
	local widgets = self:widgets()
	local update_list_size = false
...
function ForgeViewElementWeaponStats:update_expertise_value(start_value, override_value)
...
ui_owner._ui_class_RealmsForgeViewElementWeaponStats = ForgeViewElementWeaponStats
return ForgeViewElementWeaponStats
