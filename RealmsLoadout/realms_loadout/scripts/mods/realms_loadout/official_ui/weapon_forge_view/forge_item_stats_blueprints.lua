...
local function _generate_blueprints_function(grid_size, optional_item)
	...
	equipped_display_name_style.font_size = 20
	...
	weapon_display_name_style.font_size = 20
	...
	local function _generate_weapon_stats_passes(action_pass_optional_item)
		...
		for ii = 1, 5 do
			...
			local bar_x = 22 + base_offset_x + bar_offset
			local value_x = 40 + base_offset_x
			local arrow_size = 14
			local arrow_gap = 4
			local value_width = 84
			local arrow_y = 20 + base_offset_y + bar_offset_y + 13
			local arrow_left_x = value_x - arrow_gap - arrow_size
			local arrow_right_x = value_x + value_width + arrow_gap
			...
			content_id = string.format("arrow_left_hotspot_%i", ii),
			pass_type = "hotspot",
			style = {
				horizontal_alignment = "left",
				vertical_alignment = "top",
		...
		pass_templates[#pass_templates + 1] = {
			...
			style_id = string.format("arrow_left_%i", ii),
			style = {
				horizontal_alignment = "left",
				vertical_alignment = "top",
				uvs = {
		...
		pass_templates[#pass_templates + 1] = {
			content_id = string.format("arrow_right_hotspot_%i", ii),
			pass_type = "hotspot",
			style = {
				horizontal_alignment = "left",
				vertical_alignment = "top",
		...
		pass_templates[#pass_templates + 1] = {
			pass_type = "texture",
			value = "content/ui/materials/buttons/arrow_01",
			style_id = string.format("arrow_right_%i", ii),
			style = {
				horizontal_alignment = "left",
				vertical_alignment = "top",
				offset = {
		...
		pass_templates[#pass_templates + 1] = {
			content_id = string.format("bar_slider_hotspot_%i", ii),
			pass_type = "hotspot",
			style = {
				horizontal_alignment = "left",
				vertical_alignment = "top",
		...
		pass_templates[#pass_templates + 1] = {
			pass_type = "logic",
			style_id = string.format("bar_slider_drag_%i", ii),
			value = function(pass, renderer, style, content, position, size)
				local value_key = string.format("slider_value_%i", ii)
				local drag_key = string.format("bar_slider_drag_%i", ii)
				local hotspot_key = string.format("bar_slider_hotspot_%i", ii)
				local input_service = renderer.input_service
				local left_hold = input_service and input_service:get("left_hold")
				local base_cursor = input_service and input_service:get("cursor")
				local inverse_scale = renderer.inverse_scale
				local cursor = base_cursor and inverse_scale and UIResolution.inverse_scale_vector(base_cursor, inverse_scale) or base_cursor

				if not left_hold or not cursor then
					if content.forge_drag_active_index == ii then
						content.forge_drag_active_index = nil
					end

					content[drag_key] = nil

					return
				end

				if content.forge_drag_active_index and content.forge_drag_active_index ~= ii then
					return
				end

				if not content[drag_key] then
					local hotspot = content[hotspot_key]
					local left_hotspot = content[string.format("arrow_left_hotspot_%i", ii)]
					local right_hotspot = content[string.format("arrow_right_hotspot_%i", ii)]
					local hovering_arrow = left_hotspot and left_hotspot.is_hover or right_hotspot and right_hotspot.is_hover

					if not (hotspot and hotspot.is_hover and not hovering_arrow) then
						return
					end

					content[drag_key] = true
					content.forge_drag_active_index = ii
				end

				local value = math.clamp((cursor[1] - (position[1] + bar_x)) / bar_width, 0, 1)
				local stat_cap = content.element and content.element.item and content.element.item.__forge_stat_cap or 1

				value = math.min(value, stat_cap)
				value = math.round(value / 0.01) * 0.01

				content[value_key] = value
			end
		}
	...
	local function _apply_stat_bar_values(widget, item)
		...
		local stat_cap = item.__forge_stat_cap or 1
		local num_stats = #comparing_stats

		for ii = 1, num_stats do
			local stat_data = comparing_stats[ii]
			local fraction = math.clamp(stat_data.fraction or 0, 0, stat_cap)
			local stat = {
				fraction = fraction,
				value = math.floor(fraction * 100 + 0.5)
			}
			local max_stat = {
				fraction = stat_cap,
				value = math.floor(stat_cap * 100 + 0.5)
			}
			...
			bar_style.color = bar_style.default_color
			...
			bar_bg_style.size[1] = math.round(bar_width * max_stat.fraction)
			...
			divider_2_style.offset[1] = bar_bg_style.offset[1] + bar_width
	...
	local blueprints = {
		...
		weapon_stats = {
			...
			init = function(parent, widget, element, callback_name)
				...
				local init_stat_cap = item and item.__forge_stat_cap or 1

				for ii = 1, 5 do
					local stat = base_stats and base_stats[ii]
					local initial_value = stat and stat.value or 0.6

					content[string.format("slider_value_%i", ii)] = math.min(initial_value, init_stat_cap)
				end
		...
		local base_stats = item and item.base_stats
		local stat_cap = item and item.__forge_stat_cap or 1
		local dirty = false

		for ii = 1, 5 do
			local slider_key = string.format("slider_value_%i", ii)
			local left_key = string.format("arrow_left_hotspot_%i", ii)
			local right_key = string.format("arrow_right_hotspot_%i", ii)
			local slider_value = content[slider_key]

			if not slider_value then
				slider_value = 0.6
				content[slider_key] = slider_value
			end

			local left = content[left_key]
			local right = content[right_key]
			local left_hold_time_key = string.format("arrow_left_hold_time_%i", ii)
			local left_repeat_time_key = string.format("arrow_left_repeat_time_%i", ii)
			local right_hold_time_key = string.format("arrow_right_hold_time_%i", ii)
			local right_repeat_time_key = string.format("arrow_right_repeat_time_%i", ii)
			local left_hold_time = content[left_hold_time_key] or 0
			local left_repeat_time = content[left_repeat_time_key] or 0
			local right_hold_time = content[right_hold_time_key] or 0
			local right_repeat_time = content[right_repeat_time_key] or 0
			local left_step = false
			local right_step = false

			if left then
				if left.on_pressed then
					left_step = true
					left_hold_time = 0
					left_repeat_time = 0
				elseif left.is_held then
					left_hold_time = left_hold_time + dt

					if left_hold_time >= 0.5 then
						left_repeat_time = left_repeat_time + dt

						if left_repeat_time >= 0.2 then
							left_step = true
							left_repeat_time = left_repeat_time - 0.2
						end
					end
				else
					left_hold_time = 0
			...
			if right then
				if right.on_pressed then
					right_step = true
					right_hold_time = 0
					right_repeat_time = 0
				elseif right.is_held then
					right_hold_time = right_hold_time + dt

					if right_hold_time >= 0.5 then
						right_repeat_time = right_repeat_time + dt

						if right_repeat_time >= 0.2 then
							right_step = true
							right_repeat_time = right_repeat_time - 0.2
						end
					end
				else
					right_hold_time = 0
			...
			content[left_hold_time_key] = left_hold_time
			content[left_repeat_time_key] = left_repeat_time
			content[right_hold_time_key] = right_hold_time
			content[right_repeat_time_key] = right_repeat_time

			if left_step then
				slider_value = math.clamp(math.round((slider_value - 0.01) / 0.01) * 0.01, 0, stat_cap)
				content[slider_key] = slider_value
			end
			...
			if stat and math.abs((stat.value or 0) - slider_value) > 0.0001 then
				stat.value = slider_value
				dirty = true
			end
		end
		...
		weapon_perk = {
			...
			update_item = function(widget, preview_perk, ui_renderer)
				local content = widget.content
				local style = widget.style
		...
		weapon_trait = {
			...
			update_item = function(widget, preview_trait, ui_renderer)
				local content = widget.content
				local style = widget.style
				local element = content.element
