...
if ui_owner._ui_class_RealmsViewElementProfilePresets then
    ui_owner._ensure_presets_compat()
    return ui_owner._ui_class_RealmsViewElementProfilePresets
end
...
local RealmsViewElementProfilePresets = class("RealmsViewElementProfilePresets", "ViewElementBase")

function RealmsViewElementProfilePresets:init(parent, draw_layer, start_scale, optional_menu_settings)
	local loadout_names_mod = get_mod("LoadoutNames")

	if loadout_names_mod and not (Definitions.widget_definitions and Definitions.widget_definitions.loadout_name_tbox) then
		loadout_names_mod:io_dofile("LoadoutNames/scripts/mods/LoadoutNames/ViewDefinitions")
	end

	RealmsViewElementProfilePresets.super.init(self, parent, draw_layer, start_scale, Definitions)
	...
	self:_set_tooltip_visibility(false, false)
...
function RealmsViewElementProfilePresets:sync_profiles_states()
	self:_sync_profile_buttons_items_status()
end

function RealmsViewElementProfilePresets:_setup_tooltip_grid()
...
function RealmsViewElementProfilePresets:_setup_intro_grid()
	local layout = {
		{
...
function RealmsViewElementProfilePresets:_setup_custom_icons_grid()
...
function RealmsViewElementProfilePresets:_setup_preset_buttons()
	...
	local active_profile_preset_id = Storage.get_active_profile_preset_id()
	local profile_presets = Storage.get_profile_presets()
...
function RealmsViewElementProfilePresets:_sync_profile_buttons_items_status()
	local profile_presets = Storage.get_profile_presets()
	local num_profile_presets = profile_presets and #profile_presets or 0
	local profile_buttons_widgets = self._profile_buttons_widgets
...
function RealmsViewElementProfilePresets:has_profile_presets()
	local profile_presets = Storage.get_profile_presets()

	return profile_presets and #profile_presets > 0
end

function RealmsViewElementProfilePresets:set_current_profile_loadout_status(show_warning, show_modified)
	self._current_profile_loadout_warning = show_warning
	self._current_profile_loadout_modified = show_modified
end

function RealmsViewElementProfilePresets:show_profile_preset_missing_items_warning(is_missing_content, is_modified_content, optional_preset_id)
...
function RealmsViewElementProfilePresets:can_add_profile_preset()
	local profile_presets = Storage.get_profile_presets()
	local active_profile_preset_id = Storage.get_active_profile_preset_id()
...
function RealmsViewElementProfilePresets:cb_add_new_profile_preset()
	...
	if not profile_presets or num_profile_presets >= ViewElementProfilePresetsSettings.max_profile_presets then
		return
	end

	Managers.event:trigger("event_player_save_changes_to_current_preset")

	local profile_preset_id = Storage.add_profile_preset()
	...
	-- Local loadout data is already persisted by Storage.
	self:_set_tooltip_visibility(self._tooltip_visibility, false)
end

function RealmsViewElementProfilePresets:_remove_profile_preset(widget, element)
	...
	local previously_active_profile_preset_id = Storage.get_active_profile_preset_id()

	self._active_profile_preset_id = nil

	Storage.remove_profile_preset(profile_preset_id)
	...
	if new_active_profile_preset_id and not Storage.get_profile_preset(new_active_profile_preset_id) then
...
function RealmsViewElementProfilePresets:cb_on_profile_preset_icon_grid_layout_changed(layout, content_blueprints, left_click_callback, right_click_callback, display_name, optional_grow_direction)
	local grid = self._profile_preset_tooltip_grid
...
function RealmsViewElementProfilePresets:cb_on_profile_preset_icon_grid_left_pressed(widget, element)
...
function RealmsViewElementProfilePresets:_update_profile_preset_tooltip_grid_position()
...
function RealmsViewElementProfilePresets:has_active_profile_preset()
	return self._active_profile_preset_id ~= nil
end

function RealmsViewElementProfilePresets:customize_active_profile_presets()
...
function RealmsViewElementProfilePresets:on_profile_preset_index_customize(index)
...
function RealmsViewElementProfilePresets:handling_input()
	if self._intro_active then
		return true
	end
...
function RealmsViewElementProfilePresets:_set_tooltip_visibility(visible, pulse_tooltip)
...
function RealmsViewElementProfilePresets:cycle_next_profile_preset()
	local profile_buttons_widgets = self._profile_buttons_widgets
	local active_profile_preset_id = self._active_profile_preset_id
...
function RealmsViewElementProfilePresets:_get_widget_by_preset_id(id)
	local profile_buttons_widgets = self._profile_buttons_widgets
...
function RealmsViewElementProfilePresets:_get_profile_preset_id_by_widget_index(index)
	local profile_buttons_widgets = self._profile_buttons_widgets
...
function RealmsViewElementProfilePresets:remove_active_profile_preset()
	local profile_buttons_widgets = self._profile_buttons_widgets
	...
	Storage.save_active_profile_preset_id(nil)
end

function RealmsViewElementProfilePresets:on_profile_preset_index_change(index, ignore_activation, on_preset_deleted, ignore_sound)
	...
	if not ignore_activation and (on_preset_deleted or profile_preset_id ~= self._active_profile_preset_id) then
		...
		Storage.save_active_profile_preset_id(profile_preset_id)
...
function RealmsViewElementProfilePresets:_update_profile_presets(input_service, dt, t)
...
function RealmsViewElementProfilePresets:_on_navigation_input_changed()
	RealmsViewElementProfilePresets.super._on_navigation_input_changed(self)
end

function RealmsViewElementProfilePresets:set_pivot_offset(x, y)
...
function RealmsViewElementProfilePresets:on_resolution_modified(scale)
	RealmsViewElementProfilePresets.super.on_resolution_modified(self, scale)
	self:_update_profile_preset_tooltip_grid_position()
...
function RealmsViewElementProfilePresets:update(dt, t, input_service)
	...
	return RealmsViewElementProfilePresets.super.update(self, dt, t, input_service)
end

function RealmsViewElementProfilePresets:draw(dt, t, ui_renderer, render_settings, input_service)
	if not self._is_inventory_ready then
		input_service = input_service:null_service()
	end
	...
	return RealmsViewElementProfilePresets.super.draw(self, dt, t, ui_renderer, render_settings, input_service)
end

function RealmsViewElementProfilePresets:_draw_widgets(dt, t, input_service, ui_renderer, render_settings)
	local profile_buttons_widgets = self._profile_buttons_widgets
	...
	RealmsViewElementProfilePresets.super._draw_widgets(self, dt, t, input_service, ui_renderer, render_settings)
end

function RealmsViewElementProfilePresets:_get_input_text(input_action)
...
function RealmsViewElementProfilePresets:set_input_actions(input_action_left, input_action_right)
...
function RealmsViewElementProfilePresets:set_is_handling_navigation_input(is_enabled)
	self._is_handling_navigation_input = is_enabled
end

function RealmsViewElementProfilePresets:_update_input_action_texts()
...
function RealmsViewElementProfilePresets:destroy(ui_renderer)
	self._profile_preset_tooltip_grid:destroy(ui_renderer)

	self._profile_preset_tooltip_grid = nil

	RealmsViewElementProfilePresets.super.destroy(self, ui_renderer)
end

function RealmsViewElementProfilePresets:is_costumization_open()
	return self._costumization_open
end

-- The class is stable for this mod instance. Optional mods may initialize
-- later, so retry discovery on require, but install each compatibility layer
-- only once. Keep official preset definitions shared for LoadoutNames.
function ui_owner._ensure_presets_compat()
    local better = get_mod("BetterLoadouts")
    if better and better.BL and not ui_owner._presets_better_installed then
        ui_owner._realms_bl_compat_class = RealmsViewElementProfilePresets
        local applied = ui_owner:io_dofile("realms_loadout/scripts/mods/realms_loadout/betterloadouts_compat")
        ui_owner._realms_bl_compat_class = nil
        ui_owner._presets_better_installed = applied == true
        -- BetterLoadouts may become ready after LoadoutNames. Its layout
        -- replaces only the button builder; retain the name decoration on it.
        if applied and ui_owner._refresh_loadoutnames_preset_setup then
            ui_owner._refresh_loadoutnames_preset_setup()
        end
    end
    local names = get_mod("LoadoutNames")
    if names and not ui_owner._presets_names_installed then
        ui_owner._realms_loadoutnames_presets_class = RealmsViewElementProfilePresets
        local applied = ui_owner:io_dofile("realms_loadout/scripts/mods/realms_loadout/loadoutnames_compat")
        ui_owner._realms_loadoutnames_presets_class = nil
        ui_owner._presets_names_installed = applied == true
    end
end
ui_owner._ensure_presets_compat()

ui_owner._ui_class_RealmsViewElementProfilePresets = RealmsViewElementProfilePresets
return RealmsViewElementProfilePresets
