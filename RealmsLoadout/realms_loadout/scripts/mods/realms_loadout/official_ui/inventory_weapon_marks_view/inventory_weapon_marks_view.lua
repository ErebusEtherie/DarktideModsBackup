...
if ui_owner._ui_class_RealmsInventoryWeaponMarksView then
    return ui_owner._ui_class_RealmsInventoryWeaponMarksView
end
...
local RealmsInventoryWeaponMarksView = class("RealmsInventoryWeaponMarksView", "ItemGridViewBase")
...
function RealmsInventoryWeaponMarksView:init(settings, context)
	...
	if selected_item then
		self._original_item = selected_item
	...
	RealmsInventoryWeaponMarksView.super.init(self, Definitions, settings, context)
...
function RealmsInventoryWeaponMarksView:_setup_forward_gui()
...
function RealmsInventoryWeaponMarksView:_destroy_forward_gui()
...
function RealmsInventoryWeaponMarksView:on_exit()
	...
	if self._equipped_item and self._presentation_item and self._equipped_item.name ~= self._presentation_item.name then
		...
		local item = self._equipped_item
		local gear = item and (item.__gear or item.gear)
		local master_data = gear and gear.masterDataInstance
		local overrides = master_data and master_data.overrides

		if item and gear_id then
			rawset(item, "__gear_id", gear_id)
		end

		if master_data and item.name then
			master_data.id = item.name
		end

		if overrides and item.base_stats then
			overrides.base_stats = table.clone_instance(item.base_stats)
		end

		local original_item = self._original_item
		local profile = self._preview_player and self._preview_player:profile()
		local updated_item

		if original_item and profile and WeaponCatalog then
			-- Weapon marks are local-only. Never call the official backend.
			-- Official items without a local catalog entry get one created from
			-- their own snapshot first; Sync Official Weapon can later restore
			-- the official state.
			local source_gear_id = original_item.gear_id

			if source_gear_id and not WeaponCatalog.is_local_weapon(original_item) then
				original_item.source_gear_id = source_gear_id
			end

			local ok, updated = pcall(WeaponCatalog.apply_local_weapon_mark, profile, original_item, item)

			if not (ok and updated) and source_gear_id and not WeaponCatalog.is_local_weapon(original_item) then
				local synced = pcall(WeaponCatalog.sync_weapon_from_official, profile, source_gear_id, original_item)

				if synced then
					original_item.source_gear_id = source_gear_id
					ok, updated = pcall(WeaponCatalog.apply_local_weapon_mark, profile, original_item, item)
				end
			end
		...
		if updated_item then
			Managers.event:trigger("event_weapon_cosmetic_updated", updated_item)
			Managers.event:trigger("event_switch_mark_complete", updated_item)
		else
			mod:warning("realms_loadout could not apply weapon mark locally for %s", tostring(gear_id))
		end
	end

	RealmsInventoryWeaponMarksView.super.on_exit(self)
end

function RealmsInventoryWeaponMarksView:on_enter()
	RealmsInventoryWeaponMarksView.super.on_enter(self)
...
function RealmsInventoryWeaponMarksView:_setup_item_grid(optional_grid_settings)
	if not self._presentation_item then
		return
	end

	RealmsInventoryWeaponMarksView.super._setup_item_grid(self, optional_grid_settings)
end

function RealmsInventoryWeaponMarksView:_generate_fake_mark_item(mark, marks)
...
function RealmsInventoryWeaponMarksView:_register_button_callbacks()
...
function RealmsInventoryWeaponMarksView:_update_equip_button_state()
...
function RealmsInventoryWeaponMarksView:present_grid_layout(layout)
...
function RealmsInventoryWeaponMarksView:cb_on_grid_entry_left_pressed(widget, element)
...
function RealmsInventoryWeaponMarksView:_setup_weapon_preview()
...
function RealmsInventoryWeaponMarksView:_setup_input_legend()
...
function RealmsInventoryWeaponMarksView:_fetch_marks()
...
function RealmsInventoryWeaponMarksView:_equip_weapon_mark()
...
function RealmsInventoryWeaponMarksView:cb_on_equip_pressed()
	self:_equip_weapon_mark()
end

function RealmsInventoryWeaponMarksView:_cb_on_close_pressed()
	Managers.ui:close_view("realms_inventory_weapon_marks_view")
end

function RealmsInventoryWeaponMarksView:_preview_element(element)
...
function RealmsInventoryWeaponMarksView:_preview_item(item)
...
function RealmsInventoryWeaponMarksView:on_back_pressed()
	Managers.ui:close_view("realms_inventory_weapon_marks_view")
end

function RealmsInventoryWeaponMarksView:_handle_input(input_service, dt, t)
...
function RealmsInventoryWeaponMarksView:update(dt, t, input_service)
	self:_update_equip_button_state()

	return RealmsInventoryWeaponMarksView.super.update(self, dt, t, input_service)
end

function RealmsInventoryWeaponMarksView:draw(dt, t, input_service, layer)
...
function RealmsInventoryWeaponMarksView:_draw_render_target()
	local ui_forward_renderer = self._ui_forward_renderer
...
function RealmsInventoryWeaponMarksView:_draw_widgets(dt, t, input_service, ui_renderer, render_settings)
...
function RealmsInventoryWeaponMarksView:on_resolution_modified(scale)
	RealmsInventoryWeaponMarksView.super.on_resolution_modified(self, scale)
	self:_update_tab_bar_position()
end

ui_owner._ui_class_RealmsInventoryWeaponMarksView = RealmsInventoryWeaponMarksView
return RealmsInventoryWeaponMarksView
