...
local native = { view_name = "realms_loadout_workspace_native_view", native = true,
...
function Router.equipment_page() return mod.workspace_page or native end
...
function Router.enable()
  registry.owners["realms_loadout"] = {
    window = mod._workspace_window_name,
    enabled = function() return mod:is_enabled() end,
    pages = function()
      local entries = {}
      for _, entry in ipairs(mod.workspace_pages or {}) do entries[#entries + 1] = entry end
      for _, entry in ipairs(mod.talent_workspace_entries and mod.talent_workspace_entries() or {}) do entries[#entries + 1] = entry end
      return entries
    end,
...
function Router.install()
  if not mod._workspace_window_name then
    mod._workspace_window_name = "realms_loadout_workspace_view"
    for _, view in ipairs({
      { "realms_loadout_workspace_view", "realms_loadoutWorkspaceShell", "workspace_shell", "inventory_background_view" },
      { "realms_loadout_workspace_native_view", "realms_loadoutWorkspaceNativeView", "workspace_native_view", "inventory_background_view" },
    }) do
      mod:add_require_path("realms_loadout/scripts/mods/realms_loadout/" .. view[3])
      ...
      settings.class, settings.path, settings.state_bound = view[2], "realms_loadout/scripts/mods/realms_loadout/" .. view[3], true
    ...
    mod:command("rl", mod:localize("workspace_command"), function() return Router.open("equipment") end)
  end
  Router.enable()
end

function Router.cleanup()
  registry.owners["realms_loadout"] = nil
