...
local REINDEX_CATALOG_LOCALIZATION_ID = 'reindex_buff_catalog'
...
function SettingsComponent:init(settings_widgets)
    ...
    self._leave_default_bar_alone = mod:get(LEAVE_DEFAULT_BAR_ALONE_SETTING_ID)
    self._group_buffs_in_categories = mod:get(GROUP_BUFFS_IN_CATEGORIES_SETTING_ID)
end
...
function SettingsComponent:_update_leave_default_bar_alone()
    local new_flag = Imgui.checkbox(mod:localize(LEAVE_DEFAULT_BAR_ALONE_SETTING_ID), self._leave_default_bar_alone)

    if new_flag ~= self._leave_default_bar_alone then
        self._leave_default_bar_alone = new_flag
        mod:set(LEAVE_DEFAULT_BAR_ALONE_SETTING_ID, self._leave_default_bar_alone)
    end
end

function SettingsComponent:_update_group_buffs_in_categories()
    local new_flag = Imgui.checkbox(mod:localize(GROUP_BUFFS_IN_CATEGORIES_SETTING_ID), self._group_buffs_in_categories)

    if new_flag ~= self._group_buffs_in_categories then
        self._group_buffs_in_categories = new_flag
        mod:set(GROUP_BUFFS_IN_CATEGORIES_SETTING_ID, self._group_buffs_in_categories)
    end
end

function SettingsComponent:_update_reindex_catalog()
    if Imgui.button(mod:localize(REINDEX_CATALOG_LOCALIZATION_ID)) then
...
function SettingsComponent:update()
    ...
    self:_update_leave_default_bar_alone()
    self:_update_group_buffs_in_categories()
