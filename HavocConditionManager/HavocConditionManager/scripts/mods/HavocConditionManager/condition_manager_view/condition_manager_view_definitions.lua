...
local stepper_templates = mod:io_dofile("HavocConditionManager/scripts/mods/HavocConditionManager/condition_manager_view/stepper_templates")

local addon_localization_keys = {
	label_add_havoc_circumstance = true,
	label_remove_havoc_circumstance = true,
	spawn_generation_mode = true,
	spawn_multiplier_title = true,
	spawn_multiplier_common = true,
	spawn_multiplier_elite = true,
	spawn_multiplier_special = true,
	spawn_multiplier_boss = true,
}

local function localize(key)
	if addon_localization_keys[key] then
		return mod:localize(key)
	end

	return base_mod:localize(key)
end

local function visibility_function_solo_unavailable(content, style)
	return not base_mod.can_start_game()
end
...
local scenegraph_definition = {
	...
	spawn_multiplier_common = {
		...
		vertical_alignment = "top",
		horizontal_alignment = "left",
		size = view_settings.spawn_multiplier_size,
		position = { 580, 800, 0 },
	},
...
local widget_definitions = {
	...
	title_text = UIWidget.create_definition({
		{
			...
			style_id = "title_text",
			pass_type = "text",
			value = localize("solo_view_title"),
	...
	offline_tip_text = UIWidget.create_definition({
		{
			...
			pass_type = "text",
			value = localize("tip_solo_offline"),
	...
	warn_info_box = UIWidget.create_definition({
		...
		},
		{
			...
			style_id = "text",
			pass_type = "text",
			value = localize("msg_not_available"),
			style = {
	...
	normal_title = UIWidget.create_definition({
		{
			...
			pass_type = "text",
			value = localize("title_normal_mode"),
	...
	havoc_title = UIWidget.create_definition({
		{
			...
			pass_type = "text",
			value = localize("title_havoc_mode"),
			style = table.merge_recursive(table.clone(UIFontSettings.header_2), {
				text_color = view_settings.sub_title_color,
			}),
		}
	...
	normal_start = UIWidget.create_definition(ButtonPassTemplates.default_button, "normal_start", {
		original_text = localize("button_start_normal"),
	}),
	...
	normal_tip_text = UIWidget.create_definition({
		{
			...
			pass_type = "text",
			value = localize("tip_invalid_combination"),
			style = table.merge_recursive(table.clone(UIFontSettings.header_5), {
				text_horizontal_alignment = "center",
				text_vertical_alignment = "top",
				offset = {
	...
	havoc_conditions_summary = UIWidget.create_definition({
		{
			value_id = "havoc_conditions_summary",
			style_id = "havoc_conditions_summary",
			pass_type = "text",
			value = "",
			style = table.merge_recursive(table.clone(UIFontSettings.header_5), {
				text_horizontal_alignment = "right",
	...
	spawn_multiplier_title = UIWidget.create_definition({
		{
			value_id = "spawn_multiplier_title",
			style_id = "spawn_multiplier_title",
			pass_type = "text",
			value = localize("spawn_multiplier_title"),
			style = table.merge_recursive(table.clone(UIFontSettings.header_4), {
...
add_dropdown("havoc_add_circumstance", "havoc_section", 580, 50)
add_dropdown("havoc_remove_circumstance", "havoc_section", 580, 170)
add_dropdown("havoc_theme_circumstance", "havoc_section", 580, 245)
add_dropdown("havoc_difficulty_circumstance", "havoc_section", 580, 365)
...
return Paging.extend_definitions({
