...
local function settings_slider_template(size, setting_width, font_size)
	local template = table.clone(
		SliderPassTemplates.settings_value_slider(size[1], size[2], setting_width, true)
	)

	for _, pass in ipairs(template) do
		if pass.value_id == "text" then
			pass.style.font_size = font_size
			pass.style.offset[1] = 15
			pass.style.size[1] = size[1] - setting_width
		end
	end
...
local modifier_slider_template = settings_slider_template(
	view_settings.slider_size,
	view_settings.slider_setting_width,
	18
)
local spawn_multiplier_slider_template = settings_slider_template(
	view_settings.spawn_multiplier_size,
	view_settings.spawn_multiplier_setting_width,
	20
)
...
local slider_init_function = function (parent, widget, entry, callback_name)
	...
	content.hcm_slider_size = entry.slider_size
	content.hcm_slider_setting_width = entry.slider_setting_width
	...
	local normalized_value = math.normalize_01(value, entry.min_value, entry.max_value)
...
local slider_update_function = function (parent, widget, input_service, dt, t)
	...
	if content.entry.modifier_name or content.entry.align_slider_to_setting then
		local slider_size = content.hcm_slider_size or view_settings.slider_size
		local slider_setting_width = content.hcm_slider_setting_width or view_settings.slider_setting_width
		local slider_area_width = slider_setting_width - (SLIDER_ENDPLATE_WIDTH * 2 + SLIDER_THUMB_SIZE)
		local header_width = slider_size[1] - slider_setting_width
...
local havoc_badge_pass_function = function (rank)
	...
	local badge_scale = view_settings.havoc_badge_size[1] / 200
	local letter_size = {
		40.95 * badge_scale,
		40.95 * badge_scale
	}
	local letter_margin = 8.57 * badge_scale
	local badge_offset_y = 56 * badge_scale
	local rank_offset_y = 104 * badge_scale
...
local blueprints = {
	...
	spawn_multiplier_slider = {
		size = view_settings.spawn_multiplier_size,
		pass_template = spawn_multiplier_slider_template,
		init = slider_init_function,
		update = slider_update_function,
	},
