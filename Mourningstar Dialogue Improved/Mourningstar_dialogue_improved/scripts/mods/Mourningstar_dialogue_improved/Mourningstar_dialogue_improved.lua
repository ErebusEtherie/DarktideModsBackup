...
local Definitions = mod:io_dofile("Mourningstar_dialogue_improved/scripts/mods/Mourningstar_dialogue_improved/Mourningstar_dialogue_improved_definitions")
...
local HudElementMissionSpeakerPopup = require("scripts/ui/hud/elements/mission_speaker_popup/hud_element_mission_speaker_popup")
...
local function _play_selected_sound_event(self, event, speaker_name, dialogue, extension, provided_wwise_route)
    local blank_dialogue_extension = _dialogue_extension()
    local dialogue_extension = extension
    ...
    if blank_dialogue_extension then
        local wwise_route = blank_dialogue_extension._dialogue_system._wwise_route_default
        ...
        if game_mode_name == "hub" then
            ...
            blank_dialogue_extension._voice_fx_preset = 0
            extension._voice_fx_preset = 0
        end

        -- use provided wwise_route
        if provided_wwise_route then
            wwise_route = provided_wwise_route
        end
        ...
        dialogue.currently_playing_event_id = blank_dialogue_extension:play_event(sound_event)
    end
end
...
DialogueSystem._play_dialogue_event_implementation = function(self, go_id, is_level_unit, level_name_hash, dialogue_id, dialogue_index, dialogue_rule_index, optional_query)
    ...
    -- disable mourningstar dialog completely if wanted...
    if game_mode_name == "hub" and mod:get("disable_mourningstar_chatter") == true then
        return
    end
    ...
    if not DEDICATED_SERVER then
        ...
        if sound_event then
            ...
            if rule and (rule.pre_wwise_event or rule.post_wwise_event) then
                ...
                for i, event in pairs(dialogue.dialogue_sequence) do
                    ...
                    if game_mode_name == "hub" and instance then
                        wwise_route = self._wwise_routes[3]
                        dialogue.currently_playing_event_id = _play_selected_sound_event(self, event, speaker_name, dialogue, extension, wwise_route)
                    elseif game_mode_name == "hub" then
                        dialogue.currently_playing_event_id = _play_selected_sound_event(self, event, speaker_name, dialogue, extension)
                    end
...
mod:hook_safe(
    CLASS.DialogueSystem, "update", function(self, context, dt, t)
...
mod:hook_safe(
    CLASS.StateLoading, "on_enter", function(self, ...)
...
local HudElementMissionSpeakerPopupDefinitions = require("scripts/ui/hud/elements/mission_speaker_popup/hud_element_mission_speaker_popup_definitions")
...
mod.customradio_update_widget = function(name_text, icon, dt)
    ...
    if instance and name_text then
        ...
        if widgets_by_name and widgets_by_name.custom_radio_name_text then
            ...
            widgets_by_name.custom_radio_popup.style.custom_radio_portrait.material_values.main_texture = icon or "content/ui/textures/icons/npc_portraits/mission_givers/default"
    ...
    elseif instance then
        ...
        if widgets_by_name and widgets_by_name.custom_radio_name_text and widgets_by_name.custom_radio_popup then
            ...
            widgets_by_name.custom_radio_popup.style.custom_radio_portrait.material_values.main_texture = "content/ui/textures/icons/npc_portraits/mission_givers/default"
...
mod.update_bar_value = function(instance)
    ...
    local anim_progress = math.min((1 + math.sin(Application.time_since_launch() * 6) * 0.03) * math.random_range(0.3, 0.8), 1)
    local bar_size = HudElementMissionSpeakerPopupSettings.bar_size
...
local add_definitions = function(definitions)
    ...
    definitions.scenegraph_definition.custom_radio_canvas = definitions.scenegraph_definition.custom_radio_canvas or {
...
mod:hook_require(
    "scripts/ui/views/inventory_view/inventory_view_definitions", function(definitions)
...
mod:hook_require(
    "scripts/ui/views/inventory_background_view/inventory_background_view_definitions", function(definitions)
        add_definitions(definitions)
    end
...
mod:hook_require(
    "scripts/ui/views/masteries_overview_view/masteries_overview_view_definitions", function(definitions)
        add_definitions(definitions)
    end
...
mod:hook_require(
    "scripts/ui/views/mastery_view/mastery_view_definitions", function(definitions)
        add_definitions(definitions)
    end
...
mod:hook_require(
    "scripts/ui/views/inventory_weapons_view/inventory_weapons_view_definitions", function(definitions)
        add_definitions(definitions)
    end
...
mod:hook_require(
    "scripts/ui/views/inventory_weapon_details_view/inventory_weapon_details_view_definitions", function(definitions)
        add_definitions(definitions)
    end
...
mod:hook_require(
    "scripts/ui/views/inventory_weapon_marks_view/inventory_weapon_marks_view_definitions", function(definitions)
        add_definitions(definitions)
    end
...
mod:hook_require(
    "scripts/ui/views/crafting_view/crafting_view_definitions", function(definitions)
        add_definitions(definitions)
    end
...
mod:hook_require(
    "scripts/ui/views/crafting_mechanicus_barter_items_view/crafting_mechanicus_barter_items_view_definitions", function(definitions)
        add_definitions(definitions)
    end
...
mod:hook_require(
    "scripts/ui/views/crafting_mechanicus_modify_options_view/crafting_mechanicus_modify_options_view_definitions", function(definitions)
        add_definitions(definitions)
    end
...
mod:hook_require(
    "scripts/ui/views/crafting_mechanicus_modify_view/crafting_mechanicus_modify_view_definitions", function(definitions)
        add_definitions(definitions)
    end
...
mod:hook_require(
    "scripts/ui/views/crafting_mechanicus_replace_perk_view/crafting_mechanicus_replace_perk_view_definitions", function(definitions)
        add_definitions(definitions)
    end
...
mod:hook_require(
    "scripts/ui/views/crafting_mechanicus_replace_trait_view/crafting_mechanicus_replace_trait_view_definitions", function(definitions)
        add_definitions(definitions)
    end
...
mod:hook_require(
    "scripts/ui/views/crafting_mechanicus_upgrade_expertise_view/crafting_mechanicus_upgrade_expertise_view_definitions", function(definitions)
        add_definitions(definitions)
    end
...
mod:hook_require(
    "scripts/ui/views/crafting_mechanicus_upgrade_item_view/crafting_mechanicus_upgrade_item_view_definitions", function(definitions)
        add_definitions(definitions)
    end
...
mod:hook_require(
    "scripts/ui/views/cosmetics_vendor_view/cosmetics_vendor_view_definitions", function(definitions)
        add_definitions(definitions)
    end
...
mod:hook_require(
    "scripts/ui/views/cosmetics_vendor_background_view/cosmetics_vendor_background_view_definitions", function(definitions)
        add_definitions(definitions)
    end
...
mod:hook_require(
    "scripts/ui/views/cosmetics_inspect_view/cosmetics_inspect_view_definitions", function(definitions)
        add_definitions(definitions)
    end
...
mod:hook_require(
    "scripts/ui/views/contracts_view/contracts_view_definitions", function(definitions)
        add_definitions(definitions)
    end
...
mod:hook_require(
    "scripts/ui/views/contracts_background_view/contracts_background_view_definitions", function(definitions)
        add_definitions(definitions)
    end
...
mod:hook_require(
    "scripts/ui/views/marks_goods_vendor_view/marks_goods_vendor_view_definitions", function(definitions)
        add_definitions(definitions)
    end
...
mod:hook_require(
    "scripts/ui/views/marks_vendor_view/marks_vendor_view_definitions", function(definitions)
        add_definitions(definitions)
    end
...
mod:hook_require(
    "scripts/ui/views/credits_goods_vendor_view/credits_goods_vendor_view_definitions", function(definitions)
        add_definitions(definitions)
    end
...
mod:hook_require(
    "scripts/ui/views/credits_vendor_background_view/credits_vendor_background_view_definitions", function(definitions)
        add_definitions(definitions)
    end
...
mod:hook_require(
    "scripts/ui/views/credits_vendor_view/credits_vendor_view_definitions", function(definitions)
        add_definitions(definitions)
    end
...
mod:hook_require(
    "scripts/ui/views/penance_overview_view/penance_overview_view_definitions", function(definitions)
        add_definitions(definitions)
    end
...
mod:hook_require(
    "scripts/ui/views/store_view/store_view_definitions", function(definitions)
        add_definitions(definitions)
    end
...
mod:hook_require(
    "scripts/ui/views/store_item_detail_view/store_item_detail_view_definitions", function(definitions)
        add_definitions(definitions)
    end
