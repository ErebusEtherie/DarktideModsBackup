return {
	mod_name = {
		en = "Servo-Skull Commander",
		ru = "Servo-Skull Commander — командир сервочерепа",
	},
	mod_description = {
	...
	dog_max_distance_desc = {
		en = "The Arbites mastiff runs to whatever it is pointed at, so it needs its own, much shorter reach than the servo-skull: a dog sent across the room leaves you alone and arrives late. Snipers and bombers get NO exemption from this limit (unlike the skull) for the same reason. Default 25 m.",
		ru = "Мастиф Арбитра бежит к цели ножками, поэтому у него свой, куда меньший радиус, чем у сервочерепа: пёс, отправленный через всю комнату, оставляет вас одного и прибегает поздно. Снайперы и бомберы для него НЕ исключение из лимита (в отличие от черепа) по той же причине. Дефолт 25 м.",
	},
	dog_cone_angle = {
	...
	dog_interval_desc = {
		en = "How often the order on the CURRENT target is re-issued. Orders are free for the Arbites (no charge cost), so this is only about keeping the dog pinned on the target.",
		ru = "Как часто переотдаётся приказ по ТЕКУЩЕЙ цели. У Арбитра приказы бесплатны (не тратят заряды), так что это только про удержание пса на цели.",
	},
	dog_switch_interval = {
	...
	dog_switch_interval_desc = {
		en = "Minimum seconds before switching to a higher-priority target. Longer than the skull's default on purpose: every switch throws away the distance the dog has already covered. It still switches instantly when the target dies or leaves range.",
		ru = "Минимум секунд до переключения на более приоритетную цель. Специально больше, чем у черепа: каждое переключение обнуляет уже пройденный псом путь. На мёртвую/ушедшую цель переключение всё равно мгновенное.",
	},
