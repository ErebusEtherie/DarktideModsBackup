-- salvage_exits_core.lua
local mod = get_mod("salvage")
local last_sanctuary = Mods.file.dofile("salvage/scripts/mods/salvage/salvage_last_sanctuary")
local exits = {}
local UIWidget = require("scripts/managers/ui/ui_widget")
local UIFontSettings = require("scripts/managers/ui/ui_font_settings")
local PlayerUnitVisualLoadout = require("scripts/extension_systems/visual_loadout/utilities/player_unit_visual_loadout")
local LEGACY_MARKER_TYPE = "expedition_exits_marker"
local MARKER_TYPE = "salvage_expedition_exits_marker"
local MARKER_TEMPLATE_VERSION = 2026062704
...
local legacy_markers_cleared = false
...
local y = rawget(value, "y") or rawget(value, 2)
local z = rawget(value, "z") or rawget(value, 3)
if type(x) == "number" and type(y) == "number" and type(z) == "number" then
return x, y, z
end
end
local ok_unbox, unboxed = pcall(function()
return value:unbox()
end)
if ok_unbox and unboxed ~= nil then
value = unboxed
end
if Vector3 and type(Vector3.to_elements) == "function" then
local ok_elements, x, y, z = pcall(Vector3.to_elements, value)
if ok_elements and type(x) == "number" and type(y) == "number" and type(z) == "number" then
return x, y, z
end
end
return nil
end
local function vector_copy(value)
...
if value == nil or not Vector3Box then
return nil
end
local ok_unbox, unboxed = pcall(function()
return value:unbox()
end)
if ok_unbox and vector_components(unboxed) ~= nil then
return value
end
local x, y, z = vector_components(value)
if x == nil then
...
local ok_box, boxed = pcall(Vector3Box, x, y, z)
return ok_box and boxed or nil
end
...
return ok_position and vector_box(position) or nil
end
...
local function warning_location_key(game_mode)
local current_index = current_location_index(game_mode)
local final_index = final_location_index(game_mode)
...
return warning_location_key(game_mode)
end
...
local function mark_sanctuary_alert_started(objective_name)
if SANCTUARY_ALERT_OBJECTIVES[objective_name] then
sanctuary_defend_latched = true
cached_sanctuary_defend_active = true
...
return vector_copy(value)
end
local function vector_key(position)
local x, y, z = vector_components(position)
if x == nil then
return nil
end
...
local position_key = vector_key(target.position)
if not position_key then
return nil
end
return tostring(target.kind) .. ":position:" .. tostring(target.level_index or "") .. ":" .. position_key
end
...
local position = vector_box(boxed_position)
...
if target.unit then
if not alive_unit(target.unit) then
...
if target.position then
local stable_position = vector_box(target.position)
if stable_position then
target.position = stable_position
return target
end
end
cached_target_after_activation = nil
...
local function poll_expedition_warnings(t)
if last_sanctuary and type(last_sanctuary.update) == "function" then
last_sanctuary.update(t or main_time(), false)
end
if not is_expedition_context() then
return false
end
...
refresh_runtime_state(t or main_time(), false)
return true
end
...
if not key then
if cached_target_after_activation == target then
cached_target_after_activation = nil
end
return
end
...
local world_position = vector_from_box(target.position)
...
Managers.event:trigger("add_world_marker_position", MARKER_TYPE, world_position, on_marker_added, marker_data)
end
end
...
local function reset_runtime_state(in_expedition, clear_runtime_key)
if in_expedition ~= nil then
mod.in_expedition = in_expedition
end
if clear_runtime_key ~= false then
...
if mod:get("last_sanctuary_warning") == nil then
mod:set("last_sanctuary_warning", true)
end
if mod.save then
pcall(mod.save, mod)
end
...
return vector_box(world_position)
end
if marker.unit and alive_unit(marker.unit) then
return interaction_unit_position(marker.unit)
end
return current_marker_target and vector_box(current_marker_target.position) or nil
end
...
local px, py, pz = vector_components(position)
local cx, cy, cz = vector_components(camera.position)
local rx, ry, rz = vector_components(camera.right)
local ux, uy, uz = vector_components(camera.up)
local fx, fy, fz = vector_components(camera.forward)
if px == nil or cx == nil or rx == nil or ux == nil or fx == nil then
return true
end
local dx = px - cx
local dy = py - cy
local dz = pz - cz
local view_x = dx * rx + dy * ry + dz * rz
local view_y = dx * ux + dy * uy + dz * uz
local view_z = dx * fx + dy * fy + dz * fz
if view_z <= 0.05 then
return false
end
...
local icon_size = colour == ORANGE and 63.8 or 58
...
if style.icon and style.icon.size then
style.icon.size[1] = icon_size
style.icon.size[2] = icon_size
end
if style.icon_shadow and style.icon_shadow.size then
style.icon_shadow.size[1] = icon_size
style.icon_shadow.size[2] = icon_size
end
...
template._salvage_template_version = MARKER_TEMPLATE_VERSION
template.size = { 96, 96 }
...
local function remove_existing_markers_of_type(marker_type)
local world_markers = world_markers_element()
...
if type(markers) ~= "table" or not Managers or not Managers.event then
return
end
local marker_ids = {}
for i = 1, #markers do
local marker = markers[i]
local marker_id = marker and marker.id
if marker_id then
marker_ids[#marker_ids + 1] = marker_id
end
end
for i = 1, #marker_ids do
Managers.event:trigger("remove_world_marker", marker_ids[i])
end
end
...
local marker_template = world_markers._marker_templates[MARKER_TYPE]
if not legacy_markers_cleared then
legacy_markers_cleared = true
remove_existing_markers_of_type(LEGACY_MARKER_TYPE)
end
if not marker_template or marker_template._salvage_template_version ~= MARKER_TEMPLATE_VERSION then
remove_existing_markers_of_type(MARKER_TYPE)
current_marker_id = nil
current_marker_key = nil
current_marker_kind = nil
current_marker_pending_frames = nil
current_marker_target = nil
last_marker_retry_t = -999
...
if last_sanctuary and type(last_sanctuary.on_all_mods_loaded) == "function" then
last_sanctuary.on_all_mods_loaded()
end
force_scan = true
force_state_update = true
last_state_update_t = -999
...
function exits.update(dt)
normalise_settings_once()
local t = main_time()
if last_sanctuary and type(last_sanctuary.update) == "function" then
last_sanctuary.update(t, false)
end
local runtime_active = update_runtime()
if runtime_active then
poll_expedition_warnings(t)
end
if not option_use_exit_icon_enabled() then
remove_current_marker()
return
...
if t - last_marker_retry_t >= MARKER_RETRY_INTERVAL then
last_marker_retry_t = t
request_marker(target)
end
end
elseif force_scan or t - last_scan_t >= TARGET_DISCOVERY_INTERVAL then
refresh_target(t)
end
end
...
elseif setting_id == "last_sanctuary_warning" and last_sanctuary and type(last_sanctuary.on_setting_changed) == "function" then
last_sanctuary.on_setting_changed(setting_id)
end
end
...
remove_current_marker()
reset_runtime_state(false, true)
if last_sanctuary and type(last_sanctuary.reset) == "function" then
...
if CLASS and CLASS.ExpeditionLogicServer then
mod:hook_safe(CLASS.ExpeditionLogicServer, "event_start_location_safe_zone_door_defence_sequence", function()
sanctuary_defend_latched = true
cached_sanctuary_defend_active = true
marker_colour_latched = ORANGE
if mod.in_expedition and option_use_exit_icon_enabled() then
...
mod:hook_safe(CLASS.ExpeditionLogicServer, "event_end_location_safe_zone_door_defence_sequence", function()
if last_sanctuary and type(last_sanctuary.update) == "function" then
last_sanctuary.update(main_time(), true)
end
sanctuary_defend_latched = true
...
mark_sanctuary_alert_started(objective_name)
try_warn_early_evacuation(objective_name)
if mod.in_expedition and option_use_exit_icon_enabled() then
if SANCTUARY_ALERT_OBJECTIVES[objective_name] or EXTRACTION_ALERT_OBJECTIVES[objective_name] then
request_immediate_refresh()
else
force_state_update = true
last_state_update_t = -999
...
mod:hook_safe(CLASS.MissionObjectiveSystem, "end_mission_objective", function(_, objective_name)
if mod.in_expedition and option_use_exit_icon_enabled() then
if SANCTUARY_ALERT_OBJECTIVES[objective_name] or EXTRACTION_ALERT_OBJECTIVES[objective_name] then
request_immediate_refresh()
else
...
if self and self._marker_templates then
self._marker_templates[MARKER_TYPE] = create_marker_template()
remove_existing_markers_of_type(LEGACY_MARKER_TYPE)
end
end)
...
remove_current_marker()
reset_runtime_state(false, true)
if last_sanctuary and type(last_sanctuary.on_unload) == "function" then
