...
local DOG = {
    ...
    COMPANION = "unit_threat_companion",
    DEBUG   = true,               -- Toggle debug logging to the DMF developer console / game log. Set false when finished.
    ...
    RESET_COOLDOWN_ON_KILL = true,-- Flag indicating whether or not the mod should reset the cooldown timer when the last tagged enemy is killed
    AUTO = {
    ...
    INTERROGATOR = nil,           -- Current interrogator smart tag target according to the server, if one exists
    ...
    INPUT = {
        ...
        COOLDOWN = {
            SERVER = 0.2,        -- Cooldown for the InputService to accept a new smart_tag action
	...
	SKULL = {
		RETARGET = false,		  -- Allow re-tagging the same enemy after the skull buff expires (~2s)
		RETARGET_CD = 0,
        CAP_MINIMUM = 0
	},
    MODE = {
...
mod.on_all_mods_loaded = function()
	...
	DOG.SKULL.RETARGET_CD = mod:get("skull_retarget_cd") or 0
    DOG.SKULL.CAP_MINIMUM = mod:get("skull_cap_minimum") or 0
	DOG.RESET_COOLDOWN_ON_KILL = mod:get("reset_cooldown_on_kill") or true
    -- Dog Settings
...
mod.on_setting_changed = function(setting_id)
    ...
    if setting_id == "filter_archetype" then
        ...
        if archetype ~= "cryptic" then
            mod:set("skull_hack", false, false)
        else
            mod:set("skull_hack", DOG.TARGETS.cryptic[filter].skull_hack, false)
        end
    ...
    elseif setting_id == "reset_cooldown_on_kill" then
        DOG.RESET_COOLDOWN_ON_KILL = mod:get(setting_id)
	...
	elseif setting_id == "skull_retarget_cd" then
		DOG.SKULL.RETARGET_CD = mod:get(setting_id)
    elseif setting_id == "skull_cap_minimum" then
        DOG.SKULL.CAP_MINIMUM = mod:get(setting_id)
    elseif setting_id == "skull_hack" then
        local filter = mod:get("filter_target") or "MANUAL"
        local archetype = mod:get("filter_archetype") or "cryptic"
        if archetype ~= "cryptic" then
            mod:set("skull_hack", false, false)
        else
            DOG.TARGETS.cryptic[filter].skull_hack = mod:get(setting_id) or false
        end
    end
    -- Tagging settings
...
mod.populate_breeds = function()
    ...
    -- Append skull to cryptic
    if DOG.TARGETS.cryptic.MANUAL.skull_hack == nil then
        DOG.TARGETS.cryptic.MANUAL.skull_hack = false
    end
    if DOG.TARGETS.cryptic.AUTO.skull_hack == nil then
        DOG.TARGETS.cryptic.AUTO.skull_hack = false
    end
    -- Update settings/UI
    local filter = mod:get("filter_target") or "MANUAL"
    local archetype = mod:get("filter_archetype") or "adamant"
...
mod.dog_hater = function()
    ...
    if DOG.OWNER == "cryptic" then
        -- hater if wielding grenades
        local player_manager = Managers and Managers.player
        local player = player_manager and player_manager:local_player_safe(1)
        local player_unit = player and player.player_unit
        local visual_loadout = player_unit and ScriptUnit.has_extension(player_unit, "visual_loadout_system")
        blitz = visual_loadout and visual_loadout._inventory_component.__data[1].slot_grenade_ability
        if blitz then
            blitz = blitz:match("([^/]+)$")
            local non_skulls = {
                grenade_arc = true,
            }
            if non_skulls[blitz] then
                return true
            end
        end
        -- hater if using force field
        local player_manager = Managers and Managers.player
        if player_manager then
            ...
            if ability_extension and ability_extension._equipped_abilities.grenade_ability then
                ...
                if blitz and non_skulls[blitz] then
                    return true
                end
            end
        end
...
mod.allowed_for_archetype = function()
    ...
    if DOG.OWNER == "adamant" then
        ...
        elseif mod.maybe_override_tag_type(mod.get_tag_type()) ~= DOG.LAST.TYPE then
            return true
        else
            return false
        end
    ...
    -- Skitarius logic
	if DOG.OWNER == "cryptic" then
		-- Always allow interrogators
		if DOG.INTERROGATOR then
			return true
		end
        -- Capacitance check
        if DOG.SKULL.CAP_MINIMUM > 0 then
            local skull_cap = mod.get_skull_cap()
            if skull_cap and skull_cap < DOG.SKULL.CAP_MINIMUM then
                return false
            end
        end
		-- Allow re-tagging the same target after the skull buff expires
		if DOG.SKULL.RETARGET_CD > 0 and not mod.dog_hater() and DOG.SERVER == DOG.LAST.UNIT then
            local desired_tag_type = mod.maybe_override_tag_type(mod.get_tag_type())
            if desired_tag_type ~= "unit_threat" then
			    return (DOG.LAST.TIME + DOG.SKULL.RETARGET_CD) <= Managers.time:time("main")
            end
		end
        -- Allow new tag if looking at a different target
        if DOG.SERVER ~= DOG.LAST.UNIT then
...
mod.get_skull_cap = function()
    local player = Managers.player:local_player_safe(1)
    local player_unit = player and player.player_unit
    local ability_extension = ScriptUnit.has_extension(player_unit, "ability_system")
    if ability_extension then
        local current_ability_charges = ability_extension:remaining_ability_charges("combat_ability")
        local cooldown = ability_extension:remaining_ability_cooldown("combat_ability")
        local max_cooldown = ability_extension:max_ability_cooldown("combat_ability")
        local current_cooldown_percentage = 100 - (cooldown / max_cooldown) * 100
        local total_percentage = 0
        if current_cooldown_percentage < 100 then
            total_percentage = (current_ability_charges * 100) + current_cooldown_percentage
        else
            total_percentage = current_ability_charges * 100
        end
...
mod.maybe_override_tag_type = function(tag_type)
    if not DOG.SERVER then return tag_type end
    local breed = ScriptUnit.has_extension(DOG.SERVER, "unit_data_system") and ScriptUnit.extension(DOG.SERVER, "unit_data_system"):breed()
    if not breed then return tag_type end
    -- Override poxbursters and sleeping daemonhosts to unit_threat regardless of settings to avoid griefing
    if breed.name == "chaos_poxwalker_bomber" then return "unit_threat" end
    if breed.name == "chaos_daemonhost" then
        ...
        local game_object_id = Managers.state.unit_spawner:game_object_id(DOG.SERVER)
        ...
        if not target_unit then return "unit_threat" end
    end
    return tag_type
end


mod.resolve_marker = function(value)
    if value == "unit_threat_adamant" then
...
mod.allowed_target = function()
    ...
    local interrogator_target = DOG.INTERROGATOR
    -- Prioritize interrogator if it exists
    if interrogator_target and DOG.OWNER == "cryptic" then
        if DOG.TARGETS.cryptic[filter].skull_hack then
            return true
        end
    end
    ...
    if DOG.OWNER == "adamant" or DOG.OWNER == "cryptic" then
        local tag_type = mod.get_tag_type()
        tag_type = mod.maybe_override_tag_type(tag_type)
...
mod.on_cooldown = function(type)
    ...
    if type == "server" then
        ...
        if DOG.OWNER == "adamant" and mod.maybe_override_tag_type(mod.get_tag_type()) == "unit_threat" then
	...
	elseif type == "auto" then
		local cooldown = DOG.INPUT.COOLDOWN.DOG
		if DOG.OWNER == "cryptic" and DOG.SKULL.RETARGET_CD > 0 and not mod.dog_hater() then
			cooldown = DOG.SKULL.RETARGET_CD
		end
		if DOG.LAST.TIME and (DOG.LAST.TIME + cooldown) > Managers.time:time("main") then
			return true
		end
	end
    return false
...
mod.ignore_tag = function(unit, breed)
    ...
    if DOG.IGNORE then
        ...
        if (DOG.OWNER == "adamant" and not mod.dog_hater()) or DOG.OWNER == "cryptic" then
            marker = DOG.COMPANION
...
-- Returns true if the cooldown should NOT be reset - should only be used when evaluating whether to reset the last dog tag after it has expired
mod.should_preserve_cd = function()
    if DOG.RESET_COOLDOWN_ON_KILL then
        return false
    end
    if DOG.OWNER == "cryptic" and not mod.dog_hater() then
        if DOG.SKULL.RETARGET_CD > 0 then
            return (Managers.time:time("main") - DOG.LAST.TIME) < DOG.SKULL.RETARGET_CD
        end
    end
    return (Managers.time:time("main") - DOG.LAST.TIME) < DOG.INPUT.COOLDOWN.DOG
end
...
mod:hook_safe(CLASS.SmartTagSystem, "update", function(self, context, dt, t, ...)
    if DOG.ENABLED then
        ...
        desired_tag = mod.maybe_override_tag_type(desired_tag)
        -- Fetch all current tags
        for tag_id, tag in pairs(self._all_tags) do
            local template = tag and tag._template
            if template and template.marker_type and (string.find(template.marker_type, "unit_threat") or template.marker_type == DOG.COMPANION) then
                ...
                if tagger_player and tagger_player == player then
                    ...
                    local valid_target = DOG.MANUAL > 0 or (marker == mod.resolve_marker(desired_tag)) and enemy and DOG.TARGETS[DOG.OWNER] and DOG.TARGETS[DOG.OWNER][mode] and DOG.TARGETS[DOG.OWNER][mode][enemy]
                    -- Ignore check
        ...
        if not self._all_tags[DOG.LAST.TAG] then
			local preserve_cd = mod.should_preserve_cd()
			if not preserve_cd then
				DOG.LAST.TAG = nil
				DOG.LAST.TYPE = ""
				DOG.LAST.TIME = 0
				DOG.LAST.UNIT = nil
...
mod:hook_safe(CLASS.HudElementSmartTagging, "update", function (self, dt, t, ui_renderer, render_settings, input_service)
    target_marker, target_unit, target_position = self:_find_best_smart_tag_interaction(ui_renderer, render_settings, true)
    if target_unit then 
        ...
        if smart_tag_extension then
            ...
            -- Interrogator detection
            local skull_strings = {
                loc_action_interaction_setup_decode = true,
                loc_action_interaction_setup_biotic_probe = true
            }
            
            if display_name and skull_strings[display_name] then
                DOG.INTERROGATOR = target_unit
                return
            else
                DOG.INTERROGATOR = nil
                return
            end
        end
    end
...
mod:hook(CLASS.InputService, "_get", function(func, self, action_name)
    if DOG.ENABLED and action_name == "smart_tag" then
        ...
        if mod.allowed_target() and not mod.on_cooldown("server") and mod.allowed_for_archetype() then
            ...
            if DOG.WHISTLE then
                return mod.set_tag(smart_tag_system, player_unit)
            end
...
mod.set_tag = function(smart_tag_system, player_unit)
    local wanted_tag = mod.maybe_override_tag_type(mod.get_tag_type())
    if wanted_tag == "unit_threat" then
        smart_tag_system:set_contextual_unit_tag(player_unit, DOG.SERVER, false)
    else
        smart_tag_system:set_contextual_unit_tag(player_unit, DOG.SERVER, true)
    end
    DOG.INPUT.SNAPSHOT = Managers.time:time("main")
    return DOG.INTERROGATOR
end