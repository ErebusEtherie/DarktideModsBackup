...
return function(mod, path_to_darklib)
	...
	local DarkLib = {
		report = function (library_name, function_name, line_number, message)
			mod:error('[DarkLib | ' .. library_name .. '] Error in ' .. function_name .. ' at line '..line_number..': ' .. message)
		end
	}
	...
	local function report_load_error(lib_type, name)
		...
		inst.report('darklib.lua', 'load_library', 'Requested library ' .. name .. ' does not exist or is not registered. Maybe you wrote a typo?')
	end
