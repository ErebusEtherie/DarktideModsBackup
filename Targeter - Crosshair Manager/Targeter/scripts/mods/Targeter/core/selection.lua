...
mod:hook_origin("HudElementCrosshair", "_get_current_crosshair_type", function(self)
    ...
    if is_melee then
        if string_find(template_name, "transonic_sword_transonic_knife") then
            weapon_class = "transonic_sword_transonic_knife_class"
        else
            weapon_class = "melee_class"
        end

        -- Throwing weapon overrides (Dual Shivs or Zealot Throwing Knives)
