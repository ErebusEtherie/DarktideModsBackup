-- File: Targeter/scripts/mods/Targeter/crosshairs/charge_up_ads_larger_dot.lua
...
local center_size = { 8, 8 } -- Increased from {4, 4} for the larger dot
local spread_distance = 10
local hit_default_distance = 10
local hit_size = { 14, 4 }

template.name = "charge_up_ads_larger_dot"
template.size = size
...
function template.update_function(parent, ui_renderer, widget, template, crosshair_settings, dt, t, draw_hit_indicator)
    -- Performance note: This function runs every frame, but the calculations are extremely lightweight
    -- standard math ops + nil protection. It will not impact framerates.
    local style = widget.style
    local hit_progress, hit_color, hit_weakspot = parent:hit_indicator()
    local yaw, pitch = parent:_spread_yaw_pitch(dt)

    -- Nil protection: Fallback to 0 if the weapon doesn't support charge levels
    local charge_level = parent:_get_current_charge_level() or 0

    if yaw and pitch then
        -- Nil protection: Fallback to 1 if no spread scalar is provided
        ...
        -- (Darktide's charge UI usually ignores spread visually, but the math is maintained here for consistency)
    end
