...
local packages_to_load = {
    ...
    "content/fx/particles/weapons/flame_staff/psyker_flame_staff_code_control",
    "content/fx/particles/enemies/cultist_flamer/cultist_flame_thrower_hit",
    "content/fx/particles/enemies/cultist_flamer/cultist_flame_thrower",
    "content/fx/particles/enemies/renegade_flamer/renegade_flame_thrower",
    "content/fx/particles/enemies/renegade_flamer/renegade_flame_thrower_hit",
    "content/fx/particles/enemies/beast_of_nurgle/bon_vomit_projectile",
    "content/fx/particles/enemies/beast_of_nurgle/bon_vomit_splatter",
...
--[[
    ...
    "flame3p" refers to the flame vfx when viewed in third person
...
function changeFlameColor(flameType, weapon)
    ...
    elseif flameType == 3 then
        ...
        iFlame = "content/fx/particles/weapons/grenades/flame_grenade_hostile_fire_lingering_green"
        ...
        Flamestaff.actions.action_shoot_flame.fx.looping_3d_sound_effect = "wwise/events/weapon/play_minion_flamethrower_green_start"
        Flamestaff.actions.action_shoot_flame.fx.stop_looping_3d_sound_effect = "wwise/events/weapon/play_minion_flamethrower_green_stop"
        Flamestaff.actions.action_shoot_charged_flame.fx.looping_3d_sound_effect = "wwise/events/weapon/play_minion_flamethrower_green_start"
        Flamestaff.actions.action_shoot_charged_flame.fx.stop_looping_3d_sound_effect = "wwise/events/weapon/play_minion_flamethrower_green_stop"
    ...
    elseif flameType == 4 then
        ...
        iFlame = "content/fx/particles/enemies/renegade_flamer/renegade_flamer_fire_lingering"
    ...
    elseif flameType == 5 then
        ...
        if mod:get("nurgle_ground") then iFlame = "content/fx/particles/enemies/beast_of_nurgle/bon_vomit_splatter"
        else iFlame = "content/fx/particles/weapons/flame_staff/psyker_flame_staff_impact_delay" end
...
local cultistFlamerSound
mod:hook_safe("ChargeEffects", "_start_effects", function(self, ...)
    --mod:echo("start")
    if self._weapon_template_charge_effects and self._weapon_template_charge_effects.sfx_source_name == "_overheat" then
        --_fx_extension:trigger_wwise_event("wwise/events/weapon/play_minion_flamethrower_green_wind_up")
        if mod:get("flamer_sound") then cultistFlamerSound = WwiseWorld.trigger_resource_event(self._wwise_world, "wwise/events/weapon/play_minion_flamethrower_green_wind_up") end
    end
end)
mod:hook_safe("ChargeEffects", "_stop_effects", function(self, ...)
    --mod:echo("stop")
    if self._weapon_template_charge_effects and self._weapon_template_charge_effects.sfx_source_name == "_overheat" and cultistFlamerSound ~= nil then
        --_fx_extension:trigger_wwise_event("wwise/events/weapon/play_minion_flamethrower_green_wind_up")
        if mod:get("flamer_sound") then WwiseWorld.stop_event(self._wwise_world, cultistFlamerSound) end
    end
end)
...
local FlamerColor
local FlamestaffColor

-- Change the speed of the flame fx movement, for orange (zealot) flame type.
function changeFlamerBurst(flameType)
    FlamerColor = mod:get("flamer_color")
    FlamestaffColor = mod:get("flamestaff_color")
    --if flameColor ~= "Orange" then
    --    return
    --end
    if flameType == 1 then
        --Pilot Light
        if FlamerColor == 1 then Flamer.actions.action_shoot.fx.stream_effect.speed = -300 end
        if FlamestaffColor == 1 then Flamestaff.actions.action_shoot_flame.fx.stream_effect.speed = -300 end
    elseif flameType == 2 then
        --Small Shots
        if FlamerColor == 1 then Flamer.actions.action_shoot.fx.stream_effect.speed = 10 end
        if FlamestaffColor == 1 then Flamestaff.actions.action_shoot_flame.fx.stream_effect.speed = 10 end
    elseif flameType == 3 then
        --Short Blast
        if FlamerColor == 1 then Flamer.actions.action_shoot.fx.stream_effect.speed = 150 end
        if FlamestaffColor == 1 then Flamestaff.actions.action_shoot_flame.fx.stream_effect.speed = 150 end
    elseif flameType == 999 then
        --Default Burst
        if FlamerColor == 1 then Flamer.actions.action_shoot.fx.stream_effect.speed = 35 end
        if FlamestaffColor == 1 then Flamestaff.actions.action_shoot_flame.fx.stream_effect.speed = 35 end
    end
end
...
function changeFlamerHeld(flameType)
    ...
    if flameType == 1 then
        --Clean Stream
        if FlamerColor == 1 then Flamer.actions.action_shoot_braced.fx.stream_effect.speed = 0 end
        if FlamestaffColor == 1 then Flamestaff.actions.action_shoot_charged_flame.fx.stream_effect.speed = 7.5 end
    elseif flameType == 2 then
        --Forceful Blaze
        if FlamerColor == 1 then Flamer.actions.action_shoot_braced.fx.stream_effect.speed = 150 end
        if FlamestaffColor == 1 then Flamestaff.actions.action_shoot_charged_flame.fx.stream_effect.speed = 150 end
    elseif flameType == 3 then
        --Rising Flame
        if FlamerColor == 1 then Flamer.actions.action_shoot_braced.fx.stream_effect.speed = 7.5 end
        if FlamestaffColor == 1 then Flamestaff.actions.action_shoot_charged_flame.fx.stream_effect.speed = 0 end
    elseif flameType == 999 then
        --Default Spread
        if FlamerColor == 1 then Flamer.actions.action_shoot_braced.fx.stream_effect.speed = 45 end
        if FlamestaffColor == 1 then Flamestaff.actions.action_shoot_charged_flame.fx.stream_effect.speed = 45 end
    end
end
...
function changeFlamestaffBurst(flameType)
    ...
    if flameType == 1 then
        --Quick Puff
        if FlamerColor == 2 then Flamer.actions.action_shoot.fx.stream_effect.speed = -300 end
        if FlamestaffColor == 2 then Flamestaff.actions.action_shoot_flame.fx.stream_effect.speed = -300 end
    elseif flameType == 2 then
        --Rapid Wisps
        if FlamerColor == 2 then Flamer.actions.action_shoot.fx.stream_effect.speed = 5 end
        if FlamestaffColor == 2 then Flamestaff.actions.action_shoot_flame.fx.stream_effect.speed = 3 end
    elseif flameType == 3 then
        --Short Blast
        if FlamerColor == 2 then Flamer.actions.action_shoot.fx.stream_effect.speed = 300 end
        if FlamestaffColor == 2 then Flamestaff.actions.action_shoot_flame.fx.stream_effect.speed = 150 end
    elseif flameType == 999 then
        --Default Burst
        if FlamerColor == 2 then Flamer.actions.action_shoot.fx.stream_effect.speed = 33 end
        if FlamestaffColor == 2 then Flamestaff.actions.action_shoot_flame.fx.stream_effect.speed = 33 end
    end
end
...
function changeFlamestaffHeld(flameType)
    ...
    if flameType == 1 then
        --Mini Match
        if FlamerColor == 2 then Flamer.actions.action_shoot_braced.fx.stream_effect.speed = 0 end
        if FlamestaffColor == 2 then Flamestaff.actions.action_shoot_charged_flame.fx.stream_effect.speed = 0 end
    elseif flameType == 2 then
        --Octane Flare
        if FlamerColor == 2 then Flamer.actions.action_shoot_braced.fx.stream_effect.speed = 150 end
        if FlamestaffColor == 2 then Flamestaff.actions.action_shoot_charged_flame.fx.stream_effect.speed = 150 end
    elseif flameType == 3 then
        --Intermittent Torch
        if FlamerColor == 2 then Flamer.actions.action_shoot_braced.fx.stream_effect.speed = 17.5 end
        if FlamestaffColor == 2 then Flamestaff.actions.action_shoot_charged_flame.fx.stream_effect.speed = 12.5 end
    elseif flameType == 999 then
        --Default Spread
        if FlamerColor == 2 then Flamer.actions.action_shoot_braced.fx.stream_effect.speed = 35 end
        if FlamestaffColor == 2 then Flamestaff.actions.action_shoot_charged_flame.fx.stream_effect.speed = 35 end
    end
end
...
function mod.on_setting_changed(setting_id)
    ...
    elseif setting_id == "nurgle_ground" then
        changeFlameColor(mod:get("flamer_color"), 1)
        changeFlameColor(mod:get("flamestaff_color"), 2)
    end
end
...
-- Revert settings when mod is disabled.
function mod.on_disabled()
    changeFlamerBurst(999)
    changeFlamerHeld(999)
    changeFlamestaffBurst(999)
    changeFlamestaffHeld(999)
    changeFlameColor(1, 1)
    changeFlameColor(2, 2)
end