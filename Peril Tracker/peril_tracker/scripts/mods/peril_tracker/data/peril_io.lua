--[[
    peril_io.lua
    File I/O for peril tracker — saves/loads sessions as JSON.
    Follows the same pattern as uptime_io.lua.
--]]

local mod  = get_mod("peril_tracker")
local DMF  = get_mod("DMF")
local json = mod:io_dofile("peril_tracker/scripts/mods/peril_tracker/libs/json")

-- Access DMF's io table (same pattern as uptime mod)
local _io = DMF:persistent_table("_peril_io")
...
mod.appdata_path = function(self)
    ...
    return appdata .. "/Fatshark/Darktide/peril_tracker/"
end
...
mod.save_peril_session = function(self, session)
    ...
    local data = {
        ...
        overloads  = session.overloads or {},
        combats    = session.combats or {},
    }
    ...
    if mod:get("debug_messages") then
        mod:echo(string.format("Peril session saved: %s", file_name))
    end
end

-- ===== Load =====

mod.load_peril_session = function(self, file_name)
    local path = full_path(file_name)
...
local SENTINEL = {}   -- marker for missing files

mod.get_all_sessions = function(self, force_scan)
    -- Always ensure the directory exists before we try to scan it
    self:create_save_directory()
    ...
    for _, name in ipairs(file_names) do
        local s = self:load_peril_session(name)
        if s == nil then
...
mod.delete_peril_session = function(self, session)
    if not session or not session.file_path then return false end
    if mod.lib_os.remove(session.file_path) then
        local cache = self:get_session_cache()
        local new_cache = {}
...
-- ===== Cache (stored in mod settings) =====

mod.get_session_cache = function(self)
    return self:get("peril_session_cache") or {}
end

mod.set_session_cache = function(self, cache)
    self:set("peril_session_cache", cache)
end
