...
local function _load_options_payload()
    local data = mod:io_dofile("SimpleBuffFilter/scripts/mods/SimpleBuffFilter/SimpleBuffFilter_data")
...
local function _build_sentinels_from_ui()
    ...
    local s = {
        ...
        ["tbuff_refresh_now"]     = true,
        ["tbuff_debug_discovery"] = true,
        ["tbuff_wipe_target"]     = true,
        ["tbuff_wipe_now"]        = true,

        -- new fixed groups we own (robustness for duplicate detection)
        ["group_traits_melee"]    = true,
        ["group_traits_ranged"]   = true,
        ["group_misc_buffs"]      = true,
        ["group_buff_bars"]       = true,
    }

    -- add one group sentinel per archetype in UiSettings.archetype_font_icon
    local UiSettings = require("scripts/settings/ui/ui_settings")
    if UiSettings and type(UiSettings.archetype_font_icon) == "table" then
        for arch, _ in pairs(UiSettings.archetype_font_icon) do
