...
-- One grouped section per base archetype. ADE reads the optional `tab` metadata
-- when installed; DMF ignores it when ADE is not installed.
local function _build_archetype_group(archetype)
    local keys    = Resolve.archetype_loc_keys(archetype)
    local name    = (keys and keys.name_loc) and Localize(keys.name_loc) or archetype
...
local function _add_archetype_pair_tab(first, second)
    local label = first.label

    if second then
        label = label .. "\n" .. second.label
    end

    local tab_title = Colors.tint_text(label, first.primary)

    first.widget.tab = tab_title
    widgets[#widgets + 1] = first.widget

    if second then
        second.widget.tab = tab_title
        widgets[#widgets + 1] = second.widget
    end
end

local pending_archetype_group = nil

for _, archetype in ipairs(Resolve.sorted_archetypes()) do
    local archetype_group = _build_archetype_group(archetype)

    if pending_archetype_group then
        _add_archetype_pair_tab(pending_archetype_group, archetype_group)
        pending_archetype_group = nil
    else
        pending_archetype_group = archetype_group
    end
end

if pending_archetype_group then
    _add_archetype_pair_tab(pending_archetype_group)
end

-- ==== Weapon trait groups (shared across archetypes) =========================

local traits_icon    = ""
local traits_primary = Colors.color_arr("item_rarity_6")
local traits_title   = Colors.tint_text(traits_icon .. " " .. Localize("loc_weapon_inventory_traits_title_text"),
    traits_primary)
...
-- ==== Misc ===================================================================
do
    local misc_parent_icon  = ""
    local misc_parent_color = Colors.color_arr("ui_orange_light")
    local misc_parent_title = Colors.tint_text(
        misc_parent_icon .. " " .. Localize("loc_settings_menu_group_other_settings"),
        misc_parent_color)

    local misc_icon         = ""
    local misc_color        = Colors.color_arr("item_rarity_5")
    local misc_title        = Colors.tint_text(misc_icon .. " " .. Localize("loc_horde_tactical_overlay_category_misc"),
        misc_color)
