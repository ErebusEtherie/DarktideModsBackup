...
local function load_scrolling_speed_setting()
	if _widgets_by_name then
		local dmf_current = get_mod("DMF")
		if not dmf_current then return end
		local speed_val = dmf_current:get("dmf_options_scrolling_speed") or 100
		local dmf_scroll_speed = math.clamp(speed_val / 1000, 0.05, 0.5)
...
local function safe_get_mod(mod_name)
	local ok, m = pcall(get_mod, mod_name)
...
local ViewElementKeybindPopup =
	require("scripts/ui/view_elements/view_element_keybind_popup/view_element_keybind_popup")
...
DMFOptionsView.init = function(self, settings)
	...
	-- MODIFICATION FOR SORTMODMENU --
	local sortmodmenu = safe_get_mod("SortModMenu")
	local sortmodmenu_searchbox_enabled = sortmodmenu and sortmodmenu:get("searchbox_enabled")

	if sortmodmenu_searchbox_enabled then
		-- Move the grid down to make space for our search
		if definitions.scenegraph_definition.background then
			-- Original was 190
			definitions.scenegraph_definition.background.position[2] = 240
		end

		-- Requires needed for the search bar
		local TextInputPassTemplates = require("scripts/ui/pass_templates/text_input_pass_templates")
		local UIFontSettings = require("scripts/managers/ui/ui_font_settings")
		self._last_search_text = "" -- Initialize to empty

		-- Make the search bar and position it. Scenegraph allows it to be positioned isolated
		definitions.scenegraph_definition.mod_search = {
		...
		-- Search input
		definitions.widget_definitions.mod_search_input = require("scripts/managers/ui/ui_widget").create_definition(
			table.clone(TextInputPassTemplates.simple_input_field),
			"mod_search"
		)

		-- The "Hint" Label (placed at the same scenegraph as the input)
		local hint_style = table.clone(UIFontSettings.body)
		hint_style.text_color = { 128, 255, 255, 255 } -- Grayed out
		hint_style.offset = { 10, 0, 10 } -- Pushed slightly right and forward (Z-axis)
		hint_style.vertical_alignment = "center"

		definitions.widget_definitions.mod_search_hint = require("scripts/managers/ui/ui_widget").create_definition({
			{
				value = "Search Mods...",
				pass_type = "text",
				style = hint_style,
...
DMFOptionsView._map_validations = function(self, config)
	...
	for _, setting in ipairs(config_settings) do
		...
		local cat = categories[setting.category]
		if cat then
			cat.settings[setting.display_name] = {
...
DMFOptionsView.on_exit = function(self)
	...
	_last_selected_category_entry = nil
	_last_selected_category_widget = nil
		...
		options = {
			{
				...
				callback = callback(function()
					if reset_function then
						...
						local settings_default_values = selected_category
							and settings_category_default_values[selected_category]
...
DMFOptionsView._setup_offscreen_gui = function(self)
	...
	self._ui_offscreen_renderer =
		ui_manager:create_renderer(class_name .. "_ui_offscreen_renderer", self._offscreen_world)
end
			...
			if grid:is_widget_visible(widget) then
				...
				if hotspot then
					...
					if
						...
						and (
							widget.content.entry.tooltip_text
							or widget.content.entry.disabled_by
								and not table.is_empty(widget.content.entry.disabled_by)
						)
					then
...
DMFOptionsView.update = function(self, dt, t, input_service, view_data)
	...
	-- MODIFICATION FOR SEARCH WIDGET FOR SORTMODMENU --
	local search_widget = self._widgets_by_name and self._widgets_by_name.mod_search_input
	local hint_widget = self._widgets_by_name and self._widgets_by_name.mod_search_hint
	local sortmodmenu = safe_get_mod("SortModMenu")
	local sortmodmenu_searchbox_enabled = sortmodmenu and sortmodmenu:get("searchbox_enabled")

	if sortmodmenu_searchbox_enabled and search_widget and hint_widget then
		local content = search_widget.content
		local current_text = content.input_text or ""

		if content.is_writing and input_service:get("left_pressed") then
			-- Check if the mouse is NOT over the search box
			if not search_widget.content.hotspot.is_hover then
				-- Kill focus
				content.is_writing = false
			end
		end

		-- Hint visibility (make it visible if there is no text, invisible if text)
		hint_widget.content.visible = (current_text == "")

		if self._last_search_text ~= current_text then
			self._last_search_text = current_text

			local grid = self._category_content_grid
			if grid and grid._widgets then
				local widgets = grid._widgets
				local search_query = (current_text or ""):lower():gsub("%s+", "")

				local match_count = 0
				local row_height = 67

				-- PERFORM SEARCH AND REORDERING
				for i = 1, #widgets do
					local w = widgets[i]
					if w and w.content then
						local raw_name = w.content.display_name or w.content.text or ""
						local clean_name = sortmodmenu.process_mod_name(raw_name):lower():gsub("%s+", "")

						local is_match = (search_query == "") or (clean_name:find(search_query, 1, true) ~= nil)

						if is_match then
							-- Move to top based on current match index
							w.offset[2] = match_count * row_height
							match_count = match_count + 1

							if w.content.hotspot then
								w.content.hotspot.disabled = false
							end
						else
							-- Move off-screen
							w.offset[2] = 9999
							...
							if w.content.hotspot then
								w.content.hotspot.disabled = true
							end
						end
					end
				end
	...
	-- SEARCH WIDGET MODIFICATION END --
	...
	if settings_content_grid then
		local settings_grid_input_service = not category_grid_is_focused
				and not self._selected_settings_widget
				and input_service
			or input_service:null_service()
...
DMFOptionsView._update_grid_navigation_selection = function(self)
	...
	if self._using_cursor_navigation then
		...
		local selected_widget = navigation_widgets and navigation_widgets[selected_row_index]
			or self._selected_settings_widget
...
DMFOptionsView.present_category_widgets = function(self, category)
	...
	if grid_data then
		...
		self._settings_content_grid = self:_setup_grid(
			self._settings_content_widgets,
			self._settings_alignment_list,
			grid_scenegraph_id,
			grid_spacing,
			false
		)
...
DMFOptionsView._setup_category_config = function(self, config)
	-- CUSTOM VERSION OF dmf_optioms_view.lua's DMFOptionsView._setup_category_config --
	if self._category_content_widgets then
		for i = 1, #self._category_content_widgets do
			local widget = self._category_content_widgets[i]
	...
	for i = 1, #config_categories do
		...
		local valid = self._validation_mapping[category_display_name]
		valid = valid and valid.validation_result
	...
	local sortmodmenu = safe_get_mod("SortModMenu")
	local toggle_mods_display_name = dmf:localize("toggle_mods")

	if sortmodmenu then
		local function is_toggle_mods_entry(entry)
			return entry.display_name == toggle_mods_display_name
		end
		...
		for i = 0, 9 do
			...
			if pinned_mod_name and pinned_mod_name ~= "None" and pinned_mod_name ~= "" then
				pinned_priority[sortmodmenu.process_mod_name(pinned_mod_name)] = i
			end
		end

		-- Add a pin before pinned entries
		local pinned_icon = sortmodmenu:get("pinned_icon")
		local add_pinned_icon = pinned_icon and pinned_icon ~= "None" and pinned_icon ~= ""
		if add_pinned_icon then
			pinned_icon = pinned_icon .. " "
		end
		...
		for _, entry in ipairs(entries) do
			...
			local processed_name = sortmodmenu.process_mod_name(entry.display_name)
			if add_pinned_icon and pinned_priority[processed_name] then
				if not entry.display_name:find(pinned_icon, 1, true) then
					entry.display_name = pinned_icon .. entry.display_name
				end
			end
		end

		local sort_order = sortmodmenu:get("sort_order")
		if sort_order == "Descending" then
			table.sort(entries, function(a, b)
				local pri_a = pinned_priority[sortmodmenu.process_mod_name(a.display_name)]
				local pri_b = pinned_priority[sortmodmenu.process_mod_name(b.display_name)]

				if is_toggle_mods_entry(a) then
					return true
				end
				if is_toggle_mods_entry(b) then
					return false
				end

				if pri_a and pri_b then
					return pri_a < pri_b
				end
				if pri_a then
					return true
				...
				return sortmodmenu.process_mod_name(a.display_name) > sortmodmenu.process_mod_name(b.display_name)
			end)
		else
			-- Ascending (default)
			table.sort(entries, function(a, b)
		...
		-- Fallback: simple alphabetical sort if SortModMenu mod is unavailable
		table.sort(entries, function(a, b)
			if a.display_name == toggle_mods_display_name then
				return true
			end
			if b.display_name == toggle_mods_display_name then
				return false
			end
	...
	self._default_category = category_setting or (config_categories[1] and config_categories[1].display_name)
	...
	self._category_content_widgets, self._category_alignment_list =
		self:_setup_content_widgets(entries, scenegraph_id, callback_name)
	...
	self._category_content_grid = self:_setup_grid(
		self._category_content_widgets,
...
DMFOptionsView._setup_settings_config = function(self, config)
	...
	for setting_index, setting in ipairs(config_settings) do
		local cat_map = self._validation_mapping[setting.category]
		local valid = cat_map and cat_map.settings[setting.display_name] and cat_map.settings[setting.display_name].validation_result
		...
		if valid and not setting.hidden then
			...
			local widget, alignment_widget = self:_create_settings_widget_from_config(
				setting,
				category,
				widget_suffix,
				callback_name,
...
DMFOptionsView._create_settings_widget_from_config = function(
	self,
	config,
	category,
	suffix,
...
DMFOptionsView._present_keybind_popup_grid = function(self, display_name, value, cancel_keys)
	...
	popup._text_grid:present_grid_layout(
		layout,
		definitions.grid_blueprints,
		nil,
		nil,
		nil,
		nil,
		callback(popup, "cb_on_grid_layout_changed"),
		nil
	)
end
...
DMFOptionsView._set_warning_text = function(self)
	...
	warning_text.content.text =
		string.format("Warning! Input for action %s%s%s has been unassigned.", color_1, action, color_2)
end
...
DMFOptionsView._set_selected_navigation_widget = function(self, widget)
	...
	for column_index = 1, #navigation_grids do
		...
		navigation_grid:select_grid_index(
			selected_grid and selected_row or nil,
			nil,
			nil,
