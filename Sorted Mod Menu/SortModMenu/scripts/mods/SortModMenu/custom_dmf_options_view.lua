...
DMFOptionsView.init = function (self, settings)
  ...
  local sortmodmenu_searchbox_enabled = sortmodmenu:get("searchbox_enabled")
  ...
  if sortmodmenu_searchbox_enabled then 
    ...
    definitions.scenegraph_definition.mod_search = {
        vertical_alignment = "top", parent = "background", horizontal_alignment = "left",
        size = { 335, 40 }, position = { 50, -50, 1 }
    }
    -- Search input 
    definitions.widget_definitions.mod_search_input = require("scripts/managers/ui/ui_widget").create_definition(
        table.clone(TextInputPassTemplates.simple_input_field), "mod_search"
    )
...
DMFOptionsView.update = function (self, dt, t, input_service, view_data)
  ...
  local search_widget = self._widgets_by_name.mod_search_input
  local hint_widget = self._widgets_by_name.mod_search_hint
  local sortmodmenu = get_mod("SortModMenu")
  local sortmodmenu_searchbox_enabled = sortmodmenu:get("searchbox_enabled")
  ...
  if sortmodmenu_searchbox_enabled and search_widget and hint_widget then
      ...
      if self._last_search_text ~= current_text then
          ...
          if grid and grid._widgets then
              ...
              local search_query = current_text:lower():gsub("%s+", "")
              ...
              for i = 1, #widgets do
                  ...
                  if w and w.content then
                      ...
                      if is_match then
                          ...
                          if w.content.hotspot then w.content.hotspot.disabled = false end
                      else
                          -- Move off-screen
                          w.offset[2] = 9999

                          if w.content.hotspot then w.content.hotspot.disabled = true end
                      end
                  end
              end
...
DMFOptionsView._setup_category_config = function (self, config)
  ...
  if sort_order == "Ascending" then -- Ascending
    table.sort(entries, function(a, b)
      ...
      if(a.display_name == "Toggle Mods") then return true end
      if(b.display_name == "Toggle Mods") then return false end

      if pri_a and pri_b then
          return pri_a < pri_b
      end

      if pri_a then return true  end
      if pri_b then return false end
