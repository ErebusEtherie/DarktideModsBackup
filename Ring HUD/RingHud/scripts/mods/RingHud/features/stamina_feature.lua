...
function StaminaFeature.update(hud_element, widget, hud_state, hotkey_override)
    ...
    local threshold = SETTINGS.stamina_viz_threshold
    local visible   = false

    if hotkey_override then
        visible = true
    elseif (hud_state and hud_state.is_veteran_deadshot_adsing) then
        visible = true
    ...
    if not visible then
        if base_style.visible then
            base_style.visible = false
            changed = true
        end
        if edge_style.visible then
            edge_style.visible = false
            changed = true
        end
