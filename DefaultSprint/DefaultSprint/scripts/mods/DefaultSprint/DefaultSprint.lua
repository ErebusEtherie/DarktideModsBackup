...
mod.settings = mod:persistent_table("default_sprint_settings")
...
local _input_action_hook = function(func, self, action_name)
    ...
    if action_name == "left_hold" and func(self, action_name) then
        -- Disable sprint when melee is charging
        is_charging = (current_slot == "slot_primary")
    end

    -- Sprint action
    if action_name == "sprinting" then
        -- Firstly, handle sprint weapon, if it could sprint, then let's sprint
        for _, v in ipairs(SPRINT_WEAPONS) do
            if weapon_name == v then return true end
        end

        -- Secondly, check charging action, if disable charging attack then walk
        if mod.settings["disable_when_charge"] and is_charging then return false end
        -- Next, check attack action, if not charging and attack not finished, then continue walk
        if not is_charging and is_attacking then return false end
...
mod:hook_safe("ActionHandler", "start_action",
    function(self, id, action_objects, action_name, action_params, action_settings, ...)
        ...
        if action_name:find("_heavy") then
            is_charging = false
        end
