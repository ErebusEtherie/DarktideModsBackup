...
local centin_shriek_equipped = false
local has_warp_battery = false
local max_warp_charges = 4
local timer = 0
...
local auto_vent_mode = mod:get("auto_vent_mode")
local auto_vent_warp_charges = mod:get("auto_vent_warp_charges")
local auto_vent_emergency_enable = mod:get("auto_vent_emergency_enable")
local auto_vent_emergency_threshold = mod:get("auto_vent_emergency_threshold")
local auto_vent_optimized_enable = mod:get("auto_vent_optimized_enable")
local auto_centin_enable = mod:get("auto_centin_enable")
local auto_centin_threshold = mod:get("auto_centin_threshold")
...
mod.on_setting_changed = function(setting_id)
    ...
    auto_vent_mode = mod:get("auto_vent_mode")
    auto_vent_warp_charges = mod:get("auto_vent_warp_charges")
    auto_vent_emergency_enable = mod:get("auto_vent_emergency_enable")
    auto_vent_emergency_threshold = mod:get("auto_vent_emergency_threshold")
    auto_vent_optimized_enable = mod:get("auto_vent_optimized_enable")
    auto_centin_enable = mod:get("auto_centin_enable")
    auto_centin_threshold = mod:get("auto_centin_threshold")
end
...
local function update_equipped_ability_status()
    ...
    -- Detect Centin Shriek talent
    local has_centin_shriek_talent = profile.talents['psyker_shout_centin'] or 0
    if has_centin_shriek_talent == 1 then
        centin_shriek_equipped = true
    else
        centin_shriek_equipped = false
    end
    
    -- Detect Warp Battery talent (increases max warp charges from 4 to 6)
    has_warp_battery = profile.talents['psyker_warp_battery'] == 1
    max_warp_charges = has_warp_battery and 6 or 4
    
    mod:echo("WarpGod: Venting Shriek=" .. (venting_shriek_equipped and "YES" or "NO") .. 
             ", Centin Shriek=" .. (centin_shriek_equipped and "YES" or "NO") .. 
             ", Warp Battery=" .. (has_warp_battery and "YES (6 charges)" or "NO (4 charges)"))
end
...
mod:hook("InputService", "_get", function(func, self, action_name)
    ...
    if action_name == "combat_ability_pressed" and auto_ability_activation then
        ...
        -- Venting Shriek Auto Activation (Emergency Mode - prevents explosion)
        if auto_vent_emergency_enable and venting_shriek_equipped then
            local emergency_threshold = auto_vent_emergency_threshold
            if current_peril >= emergency_threshold then
                attempted_ability_usage = true
                return true
            end
        end
        
        -- Venting Shriek Auto Activation (Optimized Mode - only at max charges)
        if auto_vent_optimized_enable and venting_shriek_equipped then
            local max_charges_threshold = max_warp_charges / 6
            if current_peril >= max_charges_threshold then
                attempted_ability_usage = true
                return true
            end
        end
        
        -- Centin Shriek Auto Activation
        if auto_centin_enable and centin_shriek_equipped then
            local centin_charges_threshold = auto_centin_threshold / 6
            if current_peril >= centin_charges_threshold then
                attempted_ability_usage = true
                return true
            end
        end
